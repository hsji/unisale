								...
								pThis._beforeinput_result.forEach(function (state) {
									switch (state) {
										...
										case pThis._BeforeinputState.CONVERT_LOWER:
											pThis._beforeinput_result_data = input_value.substring(0, beginOffset) + pThis._beforeinput_result_insert_data + input_value.substring(endOffset);
											break;
										default:
											break;
									}
				...
				on_sys_compositionend_process : function (evt) {
					...
					if (pThis._is_focused) {
						...
						if (nexacro._Browser == "IE" && nexacro._BrowserVersion == 9) {
							...
							if (!pThis._evtfire_oninput_after_compositionend || pThis._evtorder_compositionend_is_last || pThis._use_event_beforeinput) {
								pThis._updateInputValue(value, true, newpos);
							}
							else {
