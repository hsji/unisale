...
if (nexacro.DesignForm) {
			...
			if (designform._selected_components.length > 0) {
				...
				for (var i = 0; i < count; i++) {
					...
					for (var j = 0; j < 8; j++) {
						...
						if (rcTracker[0] < x && rcTracker[0] + rcTracker[2] > x && rcTracker[1] < y && rcTracker[1] + rcTracker[3] > y && !(obj instanceof nexacro.Tabpage)) {
							cursor = j;
							break;
						}
					}
	...
	_pTrackerLayer._get_tracker_rect = function (v, rect) {
		if ((v == nexacro._mouse_cursor_t || v == nexacro._mouse_cursor_b) && rect[0] + rect[2] < nexacro._min_tracker_size) {
			return 0;
		}
		if ((v == nexacro._mouse_cursor_l || v == nexacro._mouse_cursor_r) && rect[1] + rect[3] < nexacro._min_tracker_size) {
			return 0;
		}
		var left = rect[0];
		var top = rect[1];
					...
					case nexacro._mouse_cursor_create_component:
						{
							...
							while (parentobj && parentobj != designform._inner_form) {
								...
								if (move_info.x2 < rect[0] + rect[2] && move_info.y2 < rect[1] + rect[3]) {
									break;
								}
	...
	_pDesignForm.is_need_snap = function (v) {
		if (((this._dot_visible && nexacro._design_option_snap_dot) || nexacro._design_option_snap_component || this._ruler_layer.is_use_guideline()) && v >= 0 && v < 9) {
			return true;
		}
		return false;
	};
