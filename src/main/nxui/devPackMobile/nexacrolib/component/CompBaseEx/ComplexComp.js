...
if (!nexacro.ComplexComponent) {
	...
	_pComplexComponent._setContentsMaxSize = function (width, height, before) {
		...
		if (control_elem && this._is_scrollable) {
			if (!nexacro._isNull(width) && nexacro._isNumber(width) && !nexacro._isNull(height) && nexacro._isNumber(height)) {
	...
	_pComplexComponent._callbackApper = function (over) {
		if (this._use_partitem) {
			if (over === undefined) {
				over = this._isRowFirst() ? this._calcItemScrollInfo(this._hscroll_pos, true) : this._calcItemScrollInfo(this._vscroll_pos, false);
			}
			if (over !== 0) {
	...
	_pComplexComponent._subsetBindRow = function (rowindex, rowitem, addlast) {
		if (this._databind) {
			return addlast ? this._databind._subaddBindRow(rowindex, rowitem) : this._databind._subinsBindRow(rowindex, rowitem);
		}
	};
