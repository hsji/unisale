...
if (!nexacro.BasicChart) {
	...
	_pBasicChart._setDatapointFormat = function () {
		...
		if (stacktype == "percent") {
			for (var i = 0; i < this._stackGroupingObjList.length; i++) {
	...
	_pBasicChart._setDatapoint = function () {
		...
		for (var i = 0; i < visibleSeriesCnt; i++) {
			var series = seriesset[i], stacktype, bargrouping = this._bargrouping, barvisible = false, linevisible = false;
	...
	_pBasicChart._getSumsStackPercent = function (stackgroupseries) {
		var series, sums = {
		}, pThis = this, series = stackgroupseries;
	...
	_pBasicChart._setGroupColumnDatapoints = function (series) {
		var rotateaxis = this.rotateaxis, barVisibleSeriesset = [], barVisibleCnt = this._getVisibleStackGroupingLength(), datapoints = series._datapoints, shiftedPoints, borderWidth, borderWidthInXabsWidth, pixelInXWidthEquivalent = 1, gridDimSize, minMaxValues, AxeSize, board = this._boardRect, boardWidth = 0, boardHeight = 0, borderWidth = this._boardBorderWidth, borderHeight = this._boardBorderHeight, position, decallage = 0, centerBarShift, isstepline = this._isstepline;
		pThis = this;
	...
	_pBasicChart._getVisibleSeries = function () {
		var length = this.seriesset.length, barvisible = false, linevisible = false, lineareavisible = false, visibleSeriesset = [];
...
if (!nexacro.ChartBasicSeriesControl) {
	...
	_pChartBasicSeriesControl._drawSeriesCurveLines = function () {
		...
		for (idx = 0; idx < len; idx += ps) {
			var x1 = points[idx - ps], y1 = points[idx - ps + 1], x2 = points[idx], y2 = points[idx + 1];
			var skipmove = false;
			if (y1 == null || y2 == null) {
				if (x1 != null || x1 != null) {
					if (lineareavisible) {
			...
			var checkpoint = checkpoints(x1, y1, x2, y2, xaxis, yaxis);
			if (checkpoint == null) {
				continue;
			}
			x1 = checkpoint[0];
		...
		aline = line;
		var path = this._drawCurveLine(aline, linestyle, lineopacity, false, skipmoves);
	...
	_pChartBasicSeriesControl._drawBar = function (x, y, b, barLeft, barRight, axisx, axisy, barlinestyle, barfillstyle, baropacity, index, item) {
		var left, right, bottom, top, tmp, rotateaxis = this._chart.rotateaxis, tickstartgap;
		...
		var rect, chart = this._chart, fillStyle = [], width = 0, height = 0, tooltip = chart.tooltip, evtInfo, item, seriesId, points = [], seriesGroup = chart._seriesGroup, highlightGroup = chart._highlightGroup;
	...
	_pChartBasicSeriesControl._drawPoint = function (x, y, radius, axisx, axisy, pointshape, _pointshape, pointlinestyle, pointfillstyle, pointopacity, index, item) {
		var chart = this._chart, point, cx, cy, points = [], evtInfo, seriesId, shiftPointX = 0, shiftPointY = 0, seriesGroup = chart._seriesGroup, rotateaxis = chart.rotateaxis, tmp, highlightGroup = chart._highlightGroup, tickstartgap;
	...
	_pChartBasicSeriesControl._drawLine = function (datapoints, xoffset, yoffset, axisx, axisy, linestyle, lineopacity, Ishighlight) {
		var points = datapoints.points, ps = datapoints.pointsize, prevx = null, prevy = null, path, shiftPointX = 0, startPointX, endPointX, shiftPointY = 0, startPointY, endPointY, ypos = 1, xpos = 0, tickstartgap, rotateaxis = this._chart.rotateaxis, stacktype = this._stacktype == "none" ? this._chart.stacktype : this._stacktype;
	...
	_pChartBasicSeriesControl._drawArea = function (datapoints, axisx, axisy, lineareafillstyle, lineareaopacity) {
		var points = datapoints.points, ps = datapoints.pointsize, bottom, i = 0, areaOpen = false, ypos = 1, xpos = 0, segmentStart = 0, segmentEnd = 0, area, tickstartgap, plength = datapoints.pointsize.length, rotateaxis = this._chart.rotateaxis;
		...
		while (true) {
			...
			if (stepend) {
				...
				if (!rotateaxis) {
					var startX1 = x1Point;
					var startX2 = axisx.p2c(x2) + shiftPointX;
					var startY1 = y1Point;
					var startY2 = axisy.p2c(y2) + shiftPointY;
			...
			if (y1 >= axisy._max && y2 >= axisy._max) {
				var startX1 = axisx.p2c(x1) + shiftPointX;
				var startX2 = axisx.p2c(x2) + shiftPointX;
				var startY1 = axisy.p2c(axisy._max) + shiftPointY;
				var startY2 = axisy.p2c(axisy._max) + shiftPointY;
				if (!rotateaxis) {
					tickstartgap = axisx._tickstartgap;
					if (tickstartgap) {
				...
				var startX1 = axisx.p2c(x1) + shiftPointX;
				var startX2 = axisx.p2c(x2) + shiftPointX;
				var startY1 = axisy.p2c(axisy._min) + shiftPointY;
				var startY2 = axisy.p2c(axisy._min) + shiftPointY;
				if (!rotateaxis) {
					tickstartgap = axisx._tickstartgap;
					if (tickstartgap) {
						startX1 += tickstartgap;
			...
			if (x1 != x1old) {
				var startX1 = axisx.p2c(x1old) + shiftPointX;
				var startY1 = axisy.p2c(y1) + shiftPointY;
				if (!rotateaxis) {
					tickstartgap = axisx._tickstartgap;
					if (tickstartgap) {
						startX1 += tickstartgap;
			...
			if (x2 != x2old) {
				if (!nullDataFlag) {
					var startX1 = axisx.p2c(x2) + shiftPointX;
					var startX2 = axisx.p2c(x2old) + shiftPointX;
					var startY1 = axisy.p2c(y2) + shiftPointY;
					var startY2 = axisy.p2c(y2) + shiftPointY;
	...
	_pChartBasicSeriesControl._drawCurveArea = function (points, vpoints, lineareafillstyle, lineareaopacity) {
		var boardRect = this._chart._boardRect, borderHeight = this._chart._boardBorderHeight, boardHeight = 0, boardWidth = 0, zeroBase = 0, xaxis = this._xaxis, yaxis = this._yaxis, rotateaxis = this._chart.rotateaxis, area;
		...
		if (!rotateaxis) {
			...
			for (var i = 0; i < pvlength; i++) {
				var x1 = vpoints[i][2][0], y1 = vpoints[i][2][1], cx = vpoints[i][2][2], cy = vpoints[i][2][3], x2 = vpoints[i][2][4], y2 = vpoints[i][2][5];

				var type = vpoints[i][3];
				if (type == "quadraticCurveTo") {
					area.quadraticCurveTo(new nexacro.Point(x1, y1), new nexacro.Point(cx, cy));
				}
	...
	_pChartBasicSeriesControl._drawBarItemText = function (left, right, bottom, top, width, height, item) {
		...
		if (!nexacro._isNull(itemText)) {
			...
			var board = this._chart._boardRect, borderWidth = this._chart._boardBorderWidth, borderHeight = this._chart._boardBorderHeight;
			textRect = itemText.getBoundRect();
			itemText.setClipPathBaseRect(itemText.x, textRect.top, board, borderWidth, borderHeight);
		}
	...
	_pChartBasicSeriesControl._drawLineItemText = function (datapoints, axisx, axisy, item) {
		...
		for (var i = 0; i < points.length; i += ps) {
			...
			if (!nexacro._isNull(itemText)) {
				...
				if (!rotateaxis) {
					var tickstartgap = axisx._tickstartgap;
					if (tickstartgap) {
						cx += tickstartgap;
					}
				}
				else {
					var tickstartgap = axisy._tickstartgap;
					if (tickstartgap) {
						cy += tickstartgap;
					}
				}
	...
	_pChartBasicSeriesControl._drawLineHighlight = function (item) {
		if (this.linetype == "curve") {
			...
			addCurveLine('quadratic', pts.slice(len - 2, len), [cp[2 * len - 10], cp[2 * len - 9], pts[len - 4], pts[len - 3]]);
			var path = this._drawCurveLine(line, highlightlinestyle, highlightlineopacity, true, skipmoves);
			...
			var highlightlinestyle = this.highlightlinestyle || "1px solid " + this._highlightcolor, linetype = this.linetype, highlightlineopacity = this.highlightlineopacity, datapoints = this._datapoints, categoryaxis = this._xaxis, valueaxis = this._yaxis, path, highlightGroup = this._chart._highlightGroup;
