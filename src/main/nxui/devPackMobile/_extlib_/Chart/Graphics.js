...
if (!nexacro._GraphicsLib) {
	...
	if (nexacro._Browser == "Runtime") {
		...
		nexacro._GraphicsLib._fragmentText = function (nexacroFont, text, maxWidth) {
			var words = text.split(' '), lines = [], tmpChars, line = "";
...
if (!nexacro._GraphicsLibArray) {
	...
	nexacro._GraphicsLibArray.lastIndexOfProp = function (array, prop, item, from, strict) {
		var i, obj, propValue;
	...
	nexacro._GraphicsLibArray.indexOfProp = function (array, prop, item, from, strict) {
		var len = array.length, from = Number(from) || 0, strict = strict || false, obj, propValue;
...
if (!nexacro._GraphicsLibObject) {
	...
	nexacro._GraphicsLibObject.Each = function (object, func, scope) {
		var p, scope = scope || object;
		for (p in object) {
...
if (!nexacro._GraphicsMathUtil) {
	...
	nexacro._GraphicsMathUtil.solveCubic = function (a, b, c, d, roots) {
		var epsilon = this._EPSILON, abs = Math.abs, sqrt = Math.sqrt, pow = Math.pow, cos = Math.cos, PI = Math.PI;
		...
		if (Math.abs(D) < epsilon) {
			...
			var sqp = Math.sqrt(p), snq = q > 0 ? 1 : -1;
			roots[0] = -snq * 2 * sqp - b;
			roots[1] = snq * sqp - b;
			return 2;
		}
		if (D < 0) {
			var sqp = Math.sqrt(p), phi = Math.acos(q / (sqp * sqp * sqp)) / 3, t = -2 * sqp, o = 2 * Math.PI / 3;
...
if (!nexacro._GraphicsHashMap) {
	...
	nexacro._GraphicsHashMap.makeHashKeyForArray = function (arr, toStringFn) {
...
if (!nexacro._GraphicsMatrix) {
	...
	_pGraphicsMatrix.applyToContext = function (ctx) {
		ctx.transform(this._scaleX, this._shearY, this._shearX, this._scaleY, this._translateX, this._translateY);
	};
...
if (!nexacro._GraphicsRectsManager) {
	...
	_pGraphicsRectsManager.contains = function (rect) {
		var l = 0, rects = this._rects, size = rects.length, r = size - 1, idx = 0;
		while (l <= r) {
			idx = (l + r) >>> 1;
			if (rect.left == rects[idx].left) {
				break;
	...
	_pGraphicsRectsManager.remove = function (rect) {
		var l = 0, rects = this._rects, size = rects.length, r = size - 1, arrayCopy = nexacro._GraphicsLibArray.arrayCopy, idx = 0;
	...
	_pGraphicsRectsManager._removeAll = function (rectsMgr) {
		var x, xChange = 0, rects = this._rects, size = rects.length, argRects = rectsMgr, _rects, argSize = argRects.length;
		var ret = false;
...
if (!nexacro._GraphicsObject) {
	...
	_pGraphicsObject.getCX = function () {
		...
		if (this instanceof nexacro.GraphicsGroup) {
			var pt = this.getBoundRect().getCenterPoint();
		...
		if (!rect) {
			...
			for (; child != null; child = child._nextsibling) {
				if (child.visible && !child.isEmpty()) {
					var rect = child.getBoundRect(false, iscurrent);
	...
	_pGraphicsObject._getItemsByFilter = function (propNm, val) {
		if (this._firstchild) {
			...
			var selectorkey = propNm + "_" + val, cached = filterCache[selectorkey], elem, seq;
			if (!cached) {
				cached = {
					pElement : this, 
					propertyNm : propNm, 
			...
			var elem = this._firstchild, elems = cached.elements, res = elems.slice(0), retvals = [];
			seq = 0;
			for (; elem; elem = elem._nextsibling) {
				if (elem._firstchild) {
					seq++;
	...
	if (nexacro._Browser == "Runtime") {
		_pGraphicsObject._applystyles = function (ctx, style) {
			...
			if (ctx) {
				var width = style._width, join = style._strokejoin, cap = style._strokecap, limit = style._miterlimit, fillColor = style._fillcolor, strokeColor = style._strokecolor, dashArray = style._dasharr, dashOffset = style._dashoffset, font = style._font;
...
if (!nexacro.GraphicsRect) {
	...
	_pGraphicsRect._drawShape = function (ctx, strokeMatrix) {
		var sz = this._size, rx = this.radiusx || 0, ry = this.radiusy || 0, w = sz.width, h = sz.height, x = 0, y = 0, mx = w / 2, my = h / 2;
		...
		if (isRounded) {
			ctx.beginPath();
			if (strokeMatrix) {
				var coords = [x + rx, y, x + w - rx, y, x + w, y, x + w, y + ry, x + w, ry, x + w, y + h - ry, x + w, y + h, x + w - rx, y + h, x + w - rx, y + h, x + rx, y + h, x, y + h, x, y + h - ry, x, y + h - ry, x, y + ry, x, y, x + rx, y, x + rx, y
				];
...
if (!nexacro.GraphicsEllipse) {
	...
	_pGraphicsEllipse._drawShape = function (ctx, strokeMatrix) {
		...
		if (w == h) {
			...
			if (strokeMatrix) {
				var coords = [-mx, 0, -mx, -cy, -cx, -my, 0, -my, cx, -my, mx, -cy, mx, 0, mx, cy, cx, my, 0, my, -cx, my, -mx, cy, -mx, 0
				];
				strokeMatrix._transformCoordinates(coords, 0, coords, 0, coords.length / 2);
			...
			if (lines) {
				...
				switch (this.verticalAlign) {
					...
					case "middle":
						...
						if (fillColor) {
							var len = lines.length, yGap = (len - 1) * lineHeight / 2;
							for (var i = 0; i < len; i++) {
					...
					case "bottom":
						...
						if (fillColor) {
							var len = lines.length, yGap = (len - 1) * lineHeight;
							for (var i = 0; i < len; i++) {
								ctx.fillText(lines[i], 0, i * lineHeight - yGap);
							}
							this._painted |= 4;
...
if (!nexacro.GraphicsImage) {
			...
			if (v) {
				...
				if (nexacro._Browser != "Runtime") {
					...
					if (cacheimages && cacheimages[rurl]) {
						...
						var w = imageobj.naturalWidth || imageobj.width, h = imageobj.naturalHeight || imageobj.height;
						...
						if (image_size) {
							var w = image_size.width;
							var h = image_size.height;
							this.imagewidth = w;
							this.imageheight = h;
							if (this.width <= 0) {
	...
	_pGraphicsImage._doingSomethingAfterImageloaded = function (w, h) {
		...
		if (sz == null) {
			...
			if (this._needRedarw) {
				this._drawflags |= 4;
				var layer = this._getOwnerLayer(), graph = layer.parent;
				graph._delayTask();
...
if (!nexacro.GraphicsPath) {
	...
	_pGraphicsPath.arcTo = function (pt, radiusPt, rotation, clockwise, large) {
		...
		var middle = from.add(to).divide(2), pt = from.subtract(middle).rotate(-rotation), x = pt.x, y = pt.y, abs = Math.abs, EPSILON = 1e-11, rx = abs(radius.width), ry = abs(radius.height), rxSq = rx * rx, rySq = ry * ry, xSq = x * x, ySq = y * y;
	...
	_pGraphicsPath._getStrokeCross = function (tolerance, pt) {
		var segs = this._segments, rect, sz = tolerance / 2;
	...
	_pGraphicsPath._getCurveBounds = function (sx, sy, ctrl1X, ctrl1Y, ctrl2X, ctrl2Y, eX, eY, rect) {
		var coords = new Array(6), prevCoords = [sx, sy, 0, 0, ctrl1X, ctrl1Y], coords = [eX, eY, ctrl2X, ctrl2Y, 0, 0], min = [sx, sy], max = [sx, sy], roots = new Array(2), strokePadding = this._strokePadding;
			...
			if (item._painting & 1) {
				...
				if (drawflags & (1 | 2 | 4)) {
					...
					if (matrixchanged) {
						...
						var curBounds = item.getGlobalBoundRect();
