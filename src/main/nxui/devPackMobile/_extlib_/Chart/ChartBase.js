...
if (!nexacro._ChartBase) {
	...
	_pChartBase._createdOrder = ["board", "categoryaxis", "seriesset", "valueaxes", "title", "legend", "crosshair", "hrangebar", "vrangebar", "selection", "tooltip"
	];
	...
	_pChartBase.updateContents = function (contents) {
		...
		nexacro._GraphicsLibArray.forEach(this._createdOrder, function (name, index) {
			...
			if (nexacro._GraphicsLib.isObject(value)) {
				...
				var valueaxes = this.valueaxes, seriesset = this.seriesset, valueaxis, series, object;
					...
					if (targets[i]["_legend"]) {
						...
						if (this._type_name == "BasicChart" || this._type_name == "BubbleChart") {
							...
							if (overitemid.search("SeriesItemText_") >= 0 || overitemid.search("SeriesPieItemLine_") >= 0) {
								...
								var seriesitem = this._GetSeriesItemFromIndex(itemindex);
	...
	_pChartBase._OverrideActionMouseMoveTouchMove = function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		...
		if (from_refer_comp && from_refer_comp._type_name == "GraphicsControl") {
			...
			var targets = from_refer_comp._getGraphicsObjectAll(canvasX, canvasY), len = 0, target, cursorVal;
			...
			if ((tooltip && tooltip.visible) || highlightvisible) {
				var targets, evt, len = 0, seriesItem;
				...
				if (seriesItem != this._overSeriesItem) {
					...
					if (this._overSeriesItem) {
						...
						if (seriesItemType) {
							...
							else if (oversearchpoint || oversearchline || oversearchetc) {
					...
					if (seriesItem) {
						...
						if (seriesItemType) {
							...
							else if (searchpoint || searchline || _firstseriesitem) {
								var chartName = this._type_name;
								if (chartName == "BasicChart") {
					...
					if (this.onrangezoomed && this.onrangezoomed._has_handlers) {
						...
						if (charttype == "BasicChart") {
							...
							if (this.selection._xaxis._type == "categoryAxis") {
								...
								var from = this.selection._xaxis._ticks[0];
								...
								var length = this.selection._xaxis._ticks.length;
								var to = this.selection._xaxis._ticks[length - 1];
								if (this.selection._type.indexOf("x") != -1 && this.rangezoom.indexOf("x") != -1) {
									if (to) {
								...
								var from = this.selection._yaxis._ticks[0];
								if (this.selection._type.indexOf("y") != -1 && this.rangezoom.indexOf("y") != -1) {
									if (from) {
										startaxisvalue.push(from.label);
									}
								...
								var length = this.selection._yaxis._ticks.length;
								var to = this.selection._yaxis._ticks[length - 1];
								if (this.selection._type.indexOf("y") != -1 && this.rangezoom.indexOf("y") != -1) {
									if (to) {
										endaxisvalue.push(to.label);
									}
	...
	_pChartBase.on_fire_user_ontouchstart = function (touchinfos, changedtouchinfos, from_comp, from_refer_comp) {
		if (touchinfos && touchinfos.length > 0) {
			...
			var ctrl_key = null;
			var shift_key = null;
			var screenX = touchinfos[0].screenx;
			var screenY = touchinfos[0].screeny;
			var canvasX = touchinfos[0].canvasx;
			var canvasY = touchinfos[0].canvasy;
			var clientX = touchinfos[0].clientx;
			var clientY = touchinfos[0].clienty;
	...
	_pChartBase.on_notify_graphicsControl_onclick = function (obj, e) {
		...
		if (this.onseriesclick && this.onseriesclick._has_handlers && ref_subcontrol.eventid == "onseriesclick") {
			var evt = new nexacro.ChartClickEventInfo(this, ref_subcontrol.eventid, e.button, e.altkey, e.ctrlkey, e.shiftkey, e.screenx, e.screeny, e.canvasx, e.canvasy, e.clientx, e.clienty, e.fromobject, ref_subcontrol.object, ref_subcontrol.index, ref_subcontrol.itemindex, ref_subcontrol.value);
...
if (!nexacro._AxisChartBase) {
	...
	_pAxisChartBase._appendValueaxis = function (contents) {
		...
		}, valueaxesLeng = -1, charttype = this._type_name, seriesset = this.seriesset, s1, s2, linevisible1 = false, linevisible2 = false;
	...
	_pAxisChartBase._setDatapoint = function () {
		...
		for (var i = 0; i < seriesset.length; i++) {
			...
			if (s) {
				...
				var stacktype = s._stacktype == "none" ? this.stacktype : s._stacktype, bargrouping = this._bargrouping, pointshape = s.pointshape;
	...
	_pAxisChartBase._arrangeVrangebar = function () {
		...
		if (vrangebar && vrangebar._group.visible) {
			var boardRect = this._boardRect, vrangebarspacing = this._vrangebarspacing || this._nullspacing, vrangebarRect, vrangebarRectHeight = 0, vrangebarWidth = 0, vrangebarHeight = 0, vrangebarLeft = 0, vrangebarTop = 0, boardBorderSize = this._boardBorderSize, boardBorderWidth = this._boardBorderWidth, boardspacing = this._boardspacing || this._nullspacing;
	...
	_pAxisChartBase._arrangeRangebarBoardRect = function () {
		var vrangebar = this.vrangebar;
		var hrangebar = this.hrangebar;
		var boardRect = this._boardRect, vrangebarspacing = this._vrangebarspacing || this._nullspacing, hrangebarspacing = this._hrangebarspacing || this._nullspacing, boardBorderSize = this._boardBorderSize, boardBorderWidth = this._boardBorderWidth, boardspacing = this._boardspacing || this._nullspacing, hrangebarRect, hrangebarRectHeight = 0, hrangebarWidth = 0, hrangebarHeight = 0, hrangebarLeft = 0, hrangebarTop = 0, vrangebarRect, vrangebarRectHeight = 0, vrangebarWidth = 0, vrangebarHeight = 0, vrangebarLeft = 0, vrangebarTop = 0;
...
if (!nexacro.ChartAxisControl) {
		...
		if (this.tickmin == undefined || this.tickmin == null) {
			...
			if (this.isDatetimeaxis()) {
				...
				if (val instanceof nexacro.Date) {
					...
					if (typeof val == "string") {
						var new_obj = new nexacro.Date(val);
						val = new_obj.getTime();
					}
					else if (typeof val == "number" && val > 0) {
						val = new Date(val);
						var new_obj = new nexacro.Date();
	...
	_pChartAxisControl._generateTimeTick = function (istotal) {
		...
		if (this.tickinterval && this._datetickunit) {
			var tickunit = this._datetickunit;
	...
	_pChartAxisControl._setPixedLabelHeight = function (clientWidth, clientHeight) {
		var changedTicks = this._changedTicks, ticklinestyle = this._ticklinestyle, boardlinestyle = this._boardlinestyle, boardlinevisible = this.boardlinevisible, ticklineWidth = 0, maxlabelheight = 0;
	...
	_pChartAxisControl._setData = function (clientWidth, clientHeight) {
		var changedTicks = this._changedTicks, ticklinestyle = this._ticklinestyle, boardlinestyle = this._boardlinestyle, boardlinevisible = this.boardlinevisible, ticklineWidth = 0, pixedlabelheight = 0;
		...
		this._charttickspace = this._tickendspace;
	...
	_pChartAxisControl._arrangeTickGroup = function (width, height) {
		var axisLine = this._axisLine, direction = this._direction, location = this._location, axislinestyle = this._axislinestyle, axislinewidth = 0, x = 0, y = 0, axisRectWidth = 0, axisRectHeight = 0, tickendspace = this._tickendspace;
		...
		var ticks = this._ticks, tick, location = this._location, value, ticksize, tickXoff = 0, tickYoff = 0, tickmin = this._min, tickmax = this._max, tickboardLine, vc = 0;
			...
			if (this._axis) {
				...
				if (this._trackbarresizing) {
					if (this._trackbarstart >= 0) {
						...
						var val = this.c2pScroll(this._trackbarstart);
						...
						if (this._enddata >= datamax) {
							...
							p1 = this.p2cScroll(datamax - this._enddata);
						}
					}
					if (this._trackbarend >= 0) {
						p2 = this._trackbarend;

						var val = this.c2pScroll(this._trackbarend);
						...
						if (this._startdata <= datamin) {
							...
							p2 = this.p2cScroll(datamax - this._startdata);
						}
					}
				}
				else {
		...
		return {
			moved : function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY) {
				...
				if (!dragged && (dragged |= distance[0] | distance[1])) {
					...
					var start = 0, end = 0, startdata = pThis._startdata, enddata = pThis._enddata, axis = pThis._axis, cursor = obj._cursor, linestyle = pThis._linestyle, borderwidth = 0, trackbarpadding = pThis._trackbarpadding, paddingLeft = 0, paddingRight = 0, paddingTop = 0, paddingBottom = 0, canvasLeft = 0, canvasTop = 0, trackbarborderwidth = 0, trackbarlinestyle = pThis._trackbarlinestyle, trackbarminsize = pThis.trackbarminsize || 50, trackbargripsize = pThis.trackbargripsize || 10, boundW = 0, boundH = 0, datamin = pThis._axis._tickmin || pThis._axis._datamin, datamax = pThis._axis._tickmax || pThis._axis._datamax, datastart = pThis.p2cScroll(datamin), dataend = pThis.p2cScroll(datamax);
					...
					if (cursor == "w-resize") {
						...
						if (pThis._isvscroll) {
							var bounds = pThis._trackbarRectBounds, startY = bounds.top, startHeight = pThis._trackbarRect.height + trackbarborderwidth, distanceY = distance[1], incImageH = pThis._incImageHeight || 0, decImageH = pThis._decImageHeight || 0;

							startY -= chart._boardRect.top;
							start = startY + distanceY;
							end = startHeight + startY + distanceY;

							var startpadding = borderwidth + paddingTop;
							var endpadding = borderwidth + paddingBottom;
							...
							var bounds = pThis._trackbarRectBounds, startX = bounds.left, startWidth = pThis._trackbarRect.width + trackbarborderwidth, distanceX = distance[0], incImageW = pThis._incImageWidth || 0, decImageW = pThis._decImageWidth || 0;
	...
	_pChartRangebarControl._setCursorY = function (obj, canvasY) {
		...
		if (id == "ChartTrackbarRect") {
			...
			if (incImageH > 0 || decImageH > 0) {
				...
				if (topGap < trackbargripsize) {
					this._applyCursor(obj, "n-resize");
				}
				else if (bottomGap < trackbargripsize) {
...
if (!nexacro.ChartSelectionControl) {
	...
	_pChartSelection._dragselectionObject = function (obj, x, y, canvasX, canvasY) {
		var pThis = this, chart = this.parent, selectionRect = this._selectionRect, origin = [x, y], prevXY = [x, y], dragged = 0, offset = [0, 0], distance = [0, 0];
		var boardRect = null;
		if (chart) {
			boardRect = chart._boardRect;
		}
		var yaxes = chart._yaxes;
		pThis._yaxis = yaxes[0];
		var xaxes = chart._xaxes;
		pThis._xaxis = xaxes[0];
		...
		return {
			...
			ended : function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY) {
				...
				var ret = pThis._updateSelection(boardRect, canvasX, canvasY);
				if (ret == true) {
					...
					if (pThis._type.indexOf("y") != -1) {
						...
						var tickgap = pThis._yaxis._tickendspace;
...
if (!nexacro.ChartCrosshairControl) {
	...
	_pChartCrosshair._setLineText = function (axis, pos, lineRect, lineText) {
		...
		if (ticks) {
			var position = axis._location, linedatapadding, label;
	...
	_pChartCrosshair._showCrosshair = function (_gridRectBound, canvasX, canvasY) {
		...
		if (xlinetext && xlinerect && tooltiptype.indexOf("x") != -1) {
			...
			var tickgap = axis._tickendspace;
			if (axis._type == "categoryAxis") {
			...
			if (sizeW > 0 && sizeH > 0 && xlinetext.text != "") {
				...
				var axisrect = axis._axisRect._bounds;

				if (position == "right") {
					textX = axisrect.right + (xlinetooltippaddingRight) + xlinetooltipgap;
					rectX = axisrect.right + xlinetooltipgap;
				}
				else {
					textX = axisrect.right - sizeW - (xlinetooltippaddingLeft) - xlinetooltipgap;
					rectX = axisrect.right - sizeW - (xlinetooltippaddingLeft + xlinetooltippaddingRight) - xlinetooltipgap;
				}
				textY = canvasY;
		...
		if (ylinetext && ylinetext && tooltiptype.indexOf("y") != -1) {
			...
			if (sizeW > 0 && sizeH > 0 && ylinetext.text != "") {
				...
				if (position == "top") {
					textY = axisrect.top - (ylinetooltippaddingTop) - ylinetooltipgap;
					rectY = axisrect.top - sizeH - (ylinetooltippaddingTop + ylinetooltippaddingBottom) - ylinetooltipgap;
				}
				else {
					textY = axisrect.top + sizeH + (ylinetooltippaddingBottom) + ylinetooltipgap;
					rectY = axisrect.top + ylinetooltipgap;
				}
...
if (!nexacro.ChartTitleControl) {
	...
	_pChartTitleControl._arrange = function (left, top) {
		...
		if (this._titleText && titleTextWidth > 0) {
			var textWidth = titleTextWidth - (this._borderWidth * 2);
			var textHeight = titleTextHeight - (this._borderWidth * 2);
			if (position == "top" || position == "bottom") {
				...
				if (titleTextHeight < subTextHeight) {
					if (textHeight > chartHeight) {
						...
						if (textalign == "center") {
							...
							textTop = titleHeight - titleTextHeight - this._borderHeight;
						}
						else if (textalign == "left") {
							textTop += this._borderHeight;
						}
			...
			if (textWidth > chartWidth) {
				var clipRectWidth = titleWidth - this._borderWidth - paddingLeft;
				var clipRectHeight = titleHeight + (this._borderWidth * 2);
		...
		if (this._subTitleText && subTextWidth > 0) {
			var textWidth = subTextWidth - (this._borderWidth * 2);
			var textHeight = subTextHeight - (this._borderWidth * 2);
			if (position == "top" || position == "bottom") {
...
if (!nexacro.ChartLegendControl) {
	...
	_pChartLegendControl._setData = function (seriesset) {
		...
		var charttype = this.parent._type_name, data = [];
		if (charttype == "BasicChart" || charttype == "BubbleChart") {
			var length = seriesset.length;
			for (var i = 0; i < length; i++) {
				var series = seriesset[i], sdata = series._data, ldata = {
				}, visible = false, fillstyle;
			...
			var length = seriesset[0]._seriesitems.length, seriesitem = seriesset[0]._seriesitems;
			for (var i = 0; i < length; i++) {
				var item = seriesitem[i], ldata = {
				};
...
if (!nexacro.ChartTooltipControl) {
	...
	_pChartTooltipControl._moveTooltip = function (obj, canvasX, canvasY, boardRect) {
		...
		if (obj._tooltipShow) {
			var preCanvasX = 0, preCanvasY = 0, offX = 0, offY = 0, tooltipPath = this._tooltipPath, tooltipText = this._tooltipText, bound, hookheight = this.hookheight, tooltipOffsetY = this._offsety;
					...
					default:
						{
							...
							if (chart.categoryaxis && chart.categoryaxis._isTimeAxis == true) {
...
if (!nexacro._getBeforeKey) {
	...
	nexacro._getBetweenKey = function (aKey, bKey) {
		var aCharCodeArr = this._getCharCodeArr(aKey), bCharCodeArr;
		if (!aCharCodeArr) {
...
if (!nexacro._DatasetMap) {
	...
	_pDatasetMap.changeKey = function (data, key, row, createDataFunc, scope) {
		var oldKey = data.key, remove = false, map = this.keyMap;
