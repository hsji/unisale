		...
		if (s) {
			...
			for (var i = 0; i < length; i++) {
				...
				if (itemslice) {
					...
					else if (s.valuelineblurangle == true) {
...
if (!nexacro.ChartPieSeriesControl) {
	...
	_pChartPieSeriesControl._rearrangeProcessItemData = function () {
		var total = 0;
	...
	_pChartPieSeriesControl._processItemData = function (data) {
		var length = data.length, slice = [], total = 0;
	...
	_pChartPieSeriesControl._drawPieSeries = function () {
		var items = this._seriesitems, isNullData = false, board = this._chart._boardRect, boardWidth = 0, boardHeight = 0, borderWidth = this._chart._boardBorderWidth, borderHeight = this._chart._boardBorderHeight, itemtextvisible = this.itemtextvisible;
	...
	_pChartPieSeriesControl._drawPie = function () {
		...
		for (var i = 0; i < allitems.length; i++) {
			var selectlength = selectedItem.length;
			if (selectlength > 0) {
			...
			if (valuedata && allitems[i]._isShow) {
				var selectlength = selectedallItem.length;
				if (selectlength > 0) {
		...
		function drawSlice (pThis, i) {
			if (items[i]._isShow) {
				...
				var valuelineblurangle = pThis.valuelineblurangle || 0;
				...
				if (items.length == 1) {
					...
					var _valuelineblurangle = pThis._valuelineblurangle;
					var valuelineblurhalfangle = pThis._valuelineblurangle / 2;
					var nextAngle = 0;
					var next2Angle = 0;
					var prevAngle = 0;
				...
				if (pThis.highlightvisible) {
					var highlightradius = pThis.highlightradius, hRadius = 0, w = h = 0, maxRadius = Math.min(pThis._chart._boardWidth, pThis._chart._boardHeight) / 2, highlightPathData;
		...
		function drawSliceBorder (pThis, i) {
			if (items[i]._isShow) {
				var angle = items[i].angle;
				var curAngle = items[i].angle * 180 / Math.PI;
				var valuelineblurangle = pThis._valuelineblurangle;
				var rStartAngle;
				var rEndAngle;
				var rStartAngle1;
				var rEndAngle1;
				...
				gradation = "linear-gradient(" + angle + "," + firstcolor + "," + secondcolor + ")";
	...
	_pChartPieSeriesControl._drawLabels = function () {
		...
		function drawLabel (item, startangle, index, pThis) {
			var labelAngle = 0, halfAngle = 0, sAngle = 0, angle = 0, eAngle = 0, chart = pThis._chart, maxRadius = Math.min(pThis._chart._boardWidth, pThis._chart._boardHeight) / 2, centerX = pThis._chart._centerLeft, centerY = pThis._chart._centerTop, itemText, itemtextguidesize = pThis._itemtextguidesize, textX = 0, textY = 0, itempointsize, pointx, pointy, itemtextguidelinestyle = pThis.itemtextguidelinestyle, itemtextguidelineopacity = pThis.itemtextguidelineopacity, guideStyle, guideline, seriesGroup = pThis._chart._seriesGroup, itemtextguidelocation = pThis._itemtextguidelocation, itemId, radius, borderwidth = pThis._borderwidth;
			...
			if (!nexacro._isNull(itemText)) {
				...
				itemText.setClipPathBaseRect(textX, textY, board, borderWidth, borderHeight);
			}
		}
	};
