...
pForm.gfnGetCommonCode = function(param, sLocale)
{
	...
	if (nexacro._quickview_formurl) {
		var objInDataset = new Dataset();
		objInDataset.set_name("ds_dummyInDs");
		this.addChild("ds_dummyInDs", objInDataset);
		var objOutDataset = new Dataset();
		objOutDataset.set_name("ds_dummyOutDs");
		this.addChild("ds_dummyOutDs", objOutDataset);
		
		objInDataset.clear();
		objInDataset.addColumn("HCL_CD", "string", 10);
		objInDataset.addColumn("SSC_CD", "string", 10);
		objInDataset.addRow();
		
		var arrList = [];
		for (var idx in param)
		{
			var nRow = objDs.findRow("HCL_CD", param[idx].code);
			if(nRow < 0) {
				if(arrList.indexOf(param[idx].code) < 0) {
					objInDataset.setColumn(0, "HCL_CD", param[idx].code);
					var strSvcId    = "getCommonCode";
					var strSvcUrl   = "SearchPopCommCodeList.do";
					var inData      = "dsCond=ds_dummyInDs";
					var outData     = "ds_dummyOutDs=dsList";
					var strArg      = "";	
					var callBackFnc = "fnCallback";
					trace("[ local 공통코드 동기 호출 ] \t" + param[idx].code);
					this.gfnTransaction( strSvcId , 	// transaction을 구분하기 위한 svc id값
										...
										false); 
					if(objDs.rowcount == 0) {
						objDs.copyData(objOutDataset);
					} else {
						objDs.appendData(objOutDataset);
					}
					arrList.push(param[idx].code);
				}
			}
		}
		trace("[ local 공통코드 동기 호출끝 ] gdsCommCode Total : \t" + objDs.rowcount);
	}	
...
//                param[idx].objid.set_codecolumn("COMM_CODE");
		...
		else 
		{
			...
			if(param[idx].dsname)
			{
				...
				var code 		 = param[idx].code;			
				objDs.filter("HCL_CD == '"+code+"'");
				var dsn = this[param[idx].dsname];
				if(dsn) 
				{
					dsn.copyData(objDs, true);
					var codecol = "CODE";
					var datacol = "CAPTION";
					//dsn.updateColID("SSC_CD", codecol);
					//dsn.updateColID("SSC_CD_KORN_NM", datacol);
					
					var selecttype = param[idx].selecttype;
					switch (selecttype)
					{
						case "A": 
							this._gfnInsertDefaultRow(dsn, "- 전체 -", datacol, selecttype, codecol, "");
							break;
						case "S": 
							this._gfnInsertDefaultRow(dsComboName, "- 선택 -", datacol, selecttype, codecol);
							break;
						case "N": 
							this._gfnInsertDefaultRow(dsComboName, "", datacol, selecttype, codecol);
							break;
					}		
				}
			} else {
	...
	try
	{
		...
		if(isNext)
		{
			if(this.fnCallBack)
			{
				this.fnCallBack.call(this, "gfnGetCode", 0);
			}
		} else {
