...
/**
* @class 특정 Component의 정합성을 체크를 하고 이상 여부를 리턴한다.
* @param  {Object} objComp	Check 대상 Component
* @param  {String} title component title
* @param  {String} checkrule - validatation rule
* @param  {Number} checkvalue - validatation value
* @param  {Boolean} bMessage - message 처리 여부
* @param  {Boolean} bFocus - focus 처리 여부
* @return {Boolean} 정상 true / 오류 false
* @example this.gfnValidationField(this.dsList, "U")
*/
pForm.gfnValidationField = function(objComp, title, checkrule, checkvalue, bMessage, bFocus)
{
	var itemValue = objComp.value;
	...
	switch(checkrule)
	{
		// 필수입력
		case "required":
			if( !itemValue || nexacro.trim((itemValue+"")).length == 0 ) {
				// {0} 의 입력값은 {1} 자리이어야 합니다.
				msg = "msg.err.validator.required";
				assign = [title];
			}
			break;
		...
		// size 크기 지정 : length:7
		case "length":
			if( (itemValue+"").length != parseInt(checkvalue)) {
				// {0} 의 입력값은 {1} 자리이어야 합니다.
				msg = "msg.err.validator.length";
				assign = [title, checkvalue];
			}
			break;
		// 해당 숫자 이하 : max:7
		case "max":
			if (parseFloat(itemValue) > parseFloat(checkvalue)) {
				// {0} 은(는) {1} 보다 작아야 합니다.
				msg = "msg.err.validator.sizemax";
				assign = [title, checkvalue];
			}
			break;
		// 해당 숫자 이상 : min:7
		case "min":
			if (parseFloat(itemValue) < parseFloat(checkvalue)) {
				// {0} 은(는) {1} 보다 커야 합니다.
				msg = "msg.err.validator.sizemin";
				assign = [title, checkvalue];
			}
			break;
		// 최대 size 크기: maxlength:7
		case "maxlength":
			if (itemValue.toString().length > parseInt(checkvalue)) {
				// {0} 항목의 최대 입력글자수를 초과하였습니다. 최대길이 : {1}
				msg = "msg.err.validator.maxlength";
				assign = [title, checkvalue];
			}
			break;
		// 최대 size 크기(Byte) : maxlengthB:3
		case "maxlengthbyte":
			if (this.lookupFunc("gfnLengthByte").call(itemValue) > parseInt(checkvalue)) {
				// {0} 항목의 최대 입력글자수를 초과하였습니다. 최대길이 : {1} byte
				msg = "msg.err.validator.maxlengthB";
				assign = [title, checkvalue];
			}
			break;
		...
		case "maxlengthdec":
			...
			// 정수 자리수
			if (((checkvalue+"").replace("-","")).length > parseInt(checkvalue)) {
				// {0} 은(는) {1} 보다 작아야 합니다.
				msg = "msg.err.validator.max";
				assign = [title + " 정수 자리수", checkvalue+"자리"];
			}
			break;
		// Decimal의 값의 길이 체크 : maxlengthdec:8:4
		case "maxlengthdecpoint":
			// 소수 자리수
			var arrLength = itemValue.toString().split(".");
			if (arrLength.length > 1) {
				if (arrLength[1].length > parseInt(checkvalue)) {
					// {0} 은(는) {1} 보다 작아야 합니다.
					msg = "msg.err.validator.max";
					assign = [title + " 소수 자리수", checkvalue+"자리"];
				}
			}
			break;
		...
		case "minlength":
			if (itemValue.length < parseInt(checkvalue)) {
				...
				msg = "msg.err.validator.minlength";
				assign = [title, checkvalue];
			}
			break;
		...
		case "minlengthbyte":
			if (this.lookupFunc("gfnLengthByte").call(itemValue) < parseInt(checkvalue)) {
				...
				msg = "msg.err.validator.minlengthB";
				assign = [title, checkvalue];
			}
			break;
		...
		case "special":		
			if (this.lookupFunc("gfnIsSpecialChar").call(itemValue)) {
				...
				msg = "msg.err.validator.notspecialchar";
				assign = [title];
			}
			break;
		// 전화번호 체크 - istel
	...
	if(msg) {
		if(bMessage) {
			this.gfnAlert(msg, assign, null, function() {
				if(bFocus) {
					objComp.setFocus();
				}
			});
...
pForm.gfnValidationCheckRule = function(itemName, itemValue, validationRuleSet, compare1, compare2)
{
	...
	switch(checkrule)
	{
		...
		// IP 체크 - isipaddr
		case "isipaddr":
			if(/^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/.test(itemValue))
			{
			} else {
				return this.gfnGetMessage("msg.err.validator.ipaddr",[titile]);				
			}
			break;			
		// 핸드폰 체크 - ismobile
		case "ismobile":
