...
*/
pForm.gfnExcelExport = function(objGrid, sSheetName, sFileName, sExportType, strRemoveCol, strTitle, bWordwrap, strUnit, bValue)
{
  ...
  //objExport.set_exportuitype("exportprogress"); 가이드
	...
	objExport.exportData(null, null, bValue);
};
		...
		if(oGrid.getCellPropertyValue(0,0,"displaytype") != "treeitemcontrol") {
			for (var i=0; i<objOutDs.getColCount(); i++) {
				...
				for( var j=i; j<colIdx; j++){
					...
					if(!sChangeId || sChangeId.indexOf("bind:") < 0 ) continue; //바인딩이 안되어 잇을 경우 건너띄기
