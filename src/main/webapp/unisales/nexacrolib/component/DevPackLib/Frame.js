...
* @class  회사전체관리자
*/ 
pForm.gfnSetAuthorityCompany = function(arrComp)
{
	var bEnable = true;
	for(var i=0,len=arrComp.length;i<len;i++) {
		arrComp[i].enable = bEnable;
		arrComp[i].value = this.gfnGetUserInfo("COMP");
	}
	// company를 모두 보이지 않게 한다 - 추후 검토
	this.divSearch.form.staCompany.fittocontents = "none";
	this.divSearch.form.staCompany.width = 0;
	this.divSearch.form.cboCompany.width = 0;
	this.divSearch.form.resetScroll();
};

pForm.gfnGetCompanyCode = function()
{
	return (this.parent.uCompanyCd || this.gfnGetUserInfo("COMP"));
};

/**
* @class  로그인 사용자정보를 가져온다
* @param  {String} col  컬럼명 없는 경우 EMP NO
* @return {String} 사용자정보 결과값
* @example this.gfnGetUserInfo("EMP_NO");
*/ 
pForm.gfnGetUserInfo = function(col)
{
	if(col == "COMP") col = "COMPANY_CD";
	// 임시
	if(nexacro._quickview_formurl) {
		if(col == "USER_ID") return "jihs";
		else if(col == "EMP_NO") return "jihs";
		else if(col == "USER_NAME") return "지해식";
		else if(col == "KORN_NM") return "지해식";
		else if(col == "DEPT_CD") return "1010020";
		else if(col == "COMPANY_CD") return "UNIDIA";
		return null;
	}
	
	var gdsUser = nexacro.getApplication().gdsUser;
	//trace(gdsUser.saveXML());
	return gdsUser.getColumn(0, col?col:"USER_ID");
};
...
pForm.gfnSetMain = function(bInit)
{
	...
	// Layout
	objApp.gvVfrs.set_separatesize("0,60,*");
...
pForm.gfnSaveMenuOpenLog = function (sType, sProg, sMenu)
{
	...
	if(nexacro.getApplication().gvSytmLogFlag != "Y") return;
