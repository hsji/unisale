...
*/ 
pForm.gfnTransaction = function(strSvcId, strSvcUrl, inData, outData, strArg, callBackFnc, isAsync, sSilent, sIgnoreError)
{
	...
	if(this.gfnIsDebug()) {
		...
		console.log('%c%s', 'font-weight: bold; color: black', 'Start Transaction');
		console.log('\tStart Transaction Info\t: ' , objSvcID);
		console.log('\t      Caller Form Info\t: [ URL : ' + this._orgurl + " ] [ Name :" + this.name + " ] [ Title :" + this.titletext + " ]");
	} else {
		nDataType = 2;
	}
...
pForm.gfnCallback = function (svcID, errorCode, errorMsg)
{
	...
	if(this.gfnIsDebug()) {
		console.log('%c%s', 'font-weight: bold; color: black', 'End Transaction');
		console.log('\tEnd Transaction Info\t: ' , objSvcID);
		var errString = '\t\t\tErrorCode\t: ' + errorCode + " \terrorMsg\t: " + errorMsg;
		if(errorCode == 0) {
			console.log('%c%s', 'color: #32681d;background:white', errString);
		} else {
			console.log('%c%s', 'color: #763d3e;background:yellow', errString);
		}
		console.log('\t\t\tElapse Time\t: ' + nElapseTime);
		console.log('%c-----------------------------------------------------------------------------------', 'color: blue');
	}
	
	// 에러 공통 처리
	if(errorCode != 0 && objSvcID.IgnoreError != "Y")
	{
	...
	if (!this.gfnIsNull(objSvcID.svcId) && objSvcID.callback != "gfnStopCallback")
	{
		...
		if (this[objSvcID.callback]) {
			...
			// next
			if (this["fnCallback"]) {
				this["fnCallback"].call(this, objSvcID.svcId, errorCode, errorMsg);
			}
		}
	}
};
