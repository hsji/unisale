...
pForm.gfnAlert = function (sMsgId, arrArg, sPopId, sCallback, arrButton, arrRtn)
{
	...
	if(nexacro._quickview_formurl &&  objApp.gdsMessage.rowcount == 0) {
		//
		trace("[DEBUG]\t" + "LOCAL용 GLOBAL MESSAGE 동기 서비스 호출");
		var objInDataset = new Dataset();
		objInDataset.set_name("ds_dummyMsgInDs");
		this.addChild("ds_dummyMsgInDs", objInDataset);
		
		objInDataset.clear();
		objInDataset.addColumn("MSGE_CD", "string", 10);
		objInDataset.addColumn("MSGE_TYPE_CD", "bigdecimal", 10);
		objInDataset.addColumn("MSGE", "string", 10);
		objInDataset.addRow();		
		
		var strSvcId    = "Search";
		var strSvcUrl   = "SearchSystemMsg.do";
		var inData      = "dsCond=ds_dummyMsgInDs";
		var outData     = "gdsMessage=dsList";
		var strArg      = "";
...
//  * @param {String} [sCallback] - 팝업콜백 (confirm성 메시지를 사용시 반드시 필요)
//  * @return N/A
//  * @example this.gfnAlert(this, "A", "확인하세요");	
//  */
