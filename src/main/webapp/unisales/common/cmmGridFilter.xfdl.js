(function()
{
    return function()
    {
        ...
        ************************************************************************************************/
         ...
         * 사용자 FUNCTION 영역
         ************************************************************************************************/
        this.fnInit = function(objGrid)
        {
        	var bUserHeader = this._gfnGridUserHeaderFlg(objGrid);
        	if (!bUserHeader) {
        		this.fnGetHeadText(objGrid);
        	} else{
        		this.fnGetUserHeader(objGrid);
        	}
        }
        ...
        */
        this.fnGetHeadText = function (objGrid)
        {
            ...
            {
        		if (objGrid.getCellProperty("Head", i, "uFunctionCol") == "N")
        		{
        			continue;
        		}

                var title  = objGrid.getCellProperty("Head", i,   "text").replace(/\r\n/g, "");
                var col    = objGrid.getCellProperty("Head", i,   "col");
                var body   = objGrid.getCellProperty("Body", nCell,   "text");

                //  콤보일때는 _text 로 처리한다.
                var display = objGrid.getCellProperty("Body", nCell, "displaytype");
                if( display == "combotext" || display=="combocontrol")
                {
                    body = "bind:"+body.substring(5)+"_text";
                }
        		else if (display == "checkboxcontrol" && this.gfnIsNull(title))
        		{
        			title = "선택";
        		}
