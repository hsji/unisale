    ...
    return function(path)
    {
        ...
        */
        	...
        	*/
        	if (!nexacro.ExtFileUpload)
        	{
        		...
        		*/
        		 ...
        		 * @return {boolean} 성공여부
        		*/
        //		_pExtFileUpload.upload = function (path, inDatasetsParam, outDatasetsParam, transferType, datatype, sourceCd, sourceSeq, statusCd)
        		_pExtFileUpload.upload = function (path, insertDs, deleteDs, transferType, datatype, sourceCd, sourceSeq, statusCd)
        		{
        			transferType = transferType || "all";
        			this.set_uploadurl(path);

        			if(this.parent.gfnIsNull(sourceCd))		return trace("SOURCE_CD를 지정하십시오.");
        			if(this.parent.gfnIsNull(sourceSeq))	return trace("SOURCE_SEQ를 지정하십시오.");
        			if(this.parent.gfnIsNull(statusCd))		statusCd="";

        			if(transferType == "all"){
        				return this.uploadAll(path, insertDs, deleteDs, datatype, sourceCd, sourceSeq, statusCd);
        			} else if(transferType == "each"){
        				// uploadAll 보고 처리필요
        // 				if (this.support.XHR2) {
        // 					return this.uploadEach(path, inDatasetsParam, outDatasetsParam, datatype, sourceCd, sourceSeq, statusCd, mapMapperId, masterMapperId);
        //
        // 				} else {
        // 					return this.uploadAll(path, inDatasetsParam, outDatasetsParam, datatype, sourceCd, sourceSeq, statusCd, mapMapperId, masterMapperId);
        // 				}
        		...
        		*/
        		 ...
        		 * @return {boolean} 성공여부
        		*/
        		_pExtFileUpload.homepageUpload = function (path, inDatasetsParam, outDatasetsParam, transferType, datatype, boardCd, blSeq, statusCd)
        		{
        			trace("HOMEPAGE upload");
        			transferType = transferType || "all";
        			this.set_uploadurl(path);

        			if(this.parent.gfnIsNull(boardCd))		return trace("BOARD_CODE를 지정하십시오.");
        			if(this.parent.gfnIsNull(blSeq))		blSeq="";
        			if(this.parent.gfnIsNull(statusCd))		statusCd="";

        			if(transferType == "all"){
        				return this.uploadAll(path, inDatasetsParam, outDatasetsParam, datatype, boardCd, blSeq, statusCd);
        			}
        			return false;
        		};
        		 ...
        		 * @return {boolean} 성공여부
        		*/
        		_pExtFileUpload.uploadAll = function (path, insertDs, deleteDs, datatype, sourceCd, sourceSeq, statusCd)
        		{
        			var uploadurl;
        			var rtn = false;
        			...
        			if (uploadurl)
        			{
        				...
        				var loadItem = new nexacro.ExtFileTransaction(uploadurl, "upload", this._form, "", "", datatype);
        				...
        				if (this.support.XHR2)
        				{
        					...
        					if (formData)
        					{
        						...
        						formData.append("sourceCd", sourceCd);
        						formData.append("sourceSeq", sourceSeq);
        						formData.append("statusCd", statusCd);
        						formData.append("insertDs", insertDs.saveXML());
        						formData.append("deleteDs", deleteDs.saveXML());
        				...
        				{
        					uploadurl +="?param_SourceCd="			+sourceCd
        							+	"&param_SourceSeq="			+sourceSeq
        							+	"&param_StatusCd="			+statusCd;
        		 ...
        		 * @return {boolean} 성공여부
        		*/
        		_pExtFileUpload.uploadEach = function (path, inDatasetsParam, outDatasetsParam, datatype, sourceCd, sourceSeq, statusCd)
        		{
        			var uploadurl;
        			var rtn = false;
        			...
        			if (uploadurl)
        			{
        				...
        				for(var i = 0; i < formDataCount; i++) {
        					sUploadurl  =uploadurl
        							+	"&param_SourceCd="			+sourceCd
        							+	"&param_SourceSeq="			+sourceSeq
        							+	"&param_StatusCd="			+statusCd[i];

        					var loadItem = new nexacro.ExtFileTransaction(sUploadurl, "upload", this._form, inDatasetsParam, outDatasetsParam, datatype);
        					//신규 추가
        					loadItem.scope = this.parent;
        					loadItem.parent = this;
        					//loadItem.on_start();
        					...
        					if (this.support.XHR2)
        					{
        						...
        						if (formData)
        						{
        							...
        							this._startCommunication(loadItem, sUploadurl, formData, formData.fileName);
        							rtn = true;
        						}
        					...
        					else
        					{
        						loadItem.appendCallback(this, this.on_loadframe_filemodule);
        						this._pExtFileItem = loadItem;
        						nexacro._submit(this._unique_id, sUploadurl);
        						rtn = true;
        					}
        	...
        	*/
        	if (!nexacro.ExtFileDownload)
        	{
        		 ...
        		 */
        		_pExtFileDownload.download = function (url, nSeq, sAddName, sSourceType)
        		{

        			if(this.parent.gfnIsNull(nSeq))			return trace("파일 SEQ를 지정하십시오.");
        			if(this.parent.gfnIsNull(sAddName))		sAddName = "";
        			if(this.parent.gfnIsNull(sSourceType))	sSourceType = "";
        			...
        			*/
        			...
        			if (downloadurl)
        			{
        				downloadurl +="&param_fm_seq="+nSeq
        							+ "&param_request_file_name="+sAddName
        							+ "&param_SourceCd="+sSourceType;
        ...
        // 		}
        	 ...
        	 */
        	nexacro.ExtFileButtonCtrl = function (id, left, top, width, height, right, bottom, parent)
        	{
        		nexacro.Button.call(this, id, left, top, width, height, right, bottom, null, null, null, null, parent);
