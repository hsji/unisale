(function()
{
    return function()
    {
        ...
        this.on_create = function()
        {
            ...
            obj = new Button("btnSave","50.72%","11","110","30",null,null,null,null,null,null,this.divBottom.form);
            obj.set_taborder("0");
            obj.set_text("메일발송");
            ...
            obj = new Button("btnPreview","42.13%","11","110","30",null,null,null,null,null,null,this.divBottom.form);
            obj.set_taborder("1");
            obj.set_text("미리보기");
            obj.set_cssclass("btn_TFDM_Save");
            obj.set_visible("true");
        ...
        // User Script
        this.registerScript("projectDocMailPu.xfdl", function() {
        /**
        *  메일발송
        ...
        */
        this.docEditor_onload = function(obj,e)
        {
        	...
        	if(type == "N") {	// 신규
        		...
        		this.dsHistory.setColumn(0, "SEND_MESSAGE", "도급계약서");
        		...
        		this.dsHistory.setColumn(0, "EXPIREMAIL_DATE", this.gfnAddDate(this.gfnGetDate(),15));
        ...
        */
        this.fnDocInfoSearch = function ()
        {
        	...
        	var outData     = "dsFieldInfo=dsFieldInfo dsFormerInfo=dsFormerInfo dsReceiverField=dsReceiverField";
        	var strArg      = "";
        ...
        */
        ...
        /**
        * @function fnMakeReceiverData  <br>
        * description 사용자정보 처리
        */
        this.fnMakeReceiverData = function()
        {
        	console.log("fnMakeReceiverData", this.opener);
        	console.log("fnMakeReceiverData", this.opener.dsList1);
        	var dsList1 = this.opener.dsList1;	// email
        	var dsList2 = this.opener.dsList2;	// reg
        	this.dsReceiver.clearData();
        	var nRow = this.dsReceiver.addRow();
        	this.dsReceiver.setColumn(nRow,  "USER_ID", dsList2.getColumn(0, "EMP_NO"));
        	this.dsReceiver.setColumn(nRow,  "USER_NAME", dsList2.getColumn(0, "EMP_NAME"));
        };
        ...
        */
        ...
        this.fnReformatDate = function(date, gb)
        {
        	var rtn = "";
        	if(gb == "1") {
        		rtn = date.substr(0,4) + "년 " + date.substr(4,2) + "월 " + date.substr(6,2) + "일";
        	} else {
        		rtn = date.substr(0,4) + "-" + date.substr(4,2) + "-" + date.substr(6,2);
        	}
        	return rtn;
        };
        this.numberWithCommas = function(x) {
        	if(!x) x = "0";
        	if(x instanceof nexacro.Decimal) x = x.valueOf();
            return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        };
        this.regnoFormat = function(x) {
        	return x.substr(0,6) + "-" + x.substr(6,1) + "******";
        };

        this.fn_GetReplaceContractNm = function(sContractNm)
        {
        	if(this.gfnIsNull(sContractNm))
        		return "";
        ...
        */
        ...
        {
        	var colinfo, colid, colName;
        	var arrColId = [];
        	var arrColName = [];
        	for(var i=0,len=this.dsFieldInfo.rowcount;i<len;i++) {
        		colid = this.dsFieldInfo.getColumn(i, "FIELD_ID");
        		colName = this.dsFieldInfo.getColumn(i, "FIELD_NAME");

        		arrColId.push(colid);
        		arrColName.push(colName);
        	...
        	var dsList1 = this.opener.dsList1;
        	var dsList2 = this.opener.dsList2;
        	var dsList3 = this.opener.dsList3;
        	var dsList4 = this.opener.dsList4;
        	var dsList5 = this.opener.dsList5;

        	var sContractAddType = dsList1.getColumn(0, "CONTRACT_ADD_TYPE");
        	var sPurchaseType 	= dsList1.getColumn(0, "PURCHASE_TYPE");

        	var nIdx = arrColName.indexOf("성명");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], dsList2.getColumn(0, "EMP_NAME"));

        	var strText = "";
        	if(sContractAddType == 0) //신규일 때
        	{
        		strText = "(주)디엑스투비(이하 “甲”)와 ";
        		strText += dsList2.getColumn(0, "EMP_NAME") + "(이하 “乙”)은 쌍방 합의에 따라 본 도급계약을 다음과 같이 체결한다.";
        	}
        	else //추가, 연장일 때
        	{
        		var svText = sContractAddType == 1 ? "추가" : sContractAddType == 2 ? "연장" : "";

        		strText = "본 계약서는 계약서번호 TBS-"
        		+ dsList1.getColumn(0, "CONTRACT_DOC_NO_PRE") + "에 대한 (주)디엑스투비"+"(이하 “甲”)와 "
        		+ dsList2.getColumn(0, "EMP_NAME") +"(이하 “乙”)의 "
        		+ svText + "계약서로써 역무기간을 제외한 모든 조건은 原 계약서 조항과 동일하다.";
        	}
        	var nIdx = arrColName.indexOf("계약내용");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], strText);

        	nIdx = arrColName.indexOf("계약번호");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], "TBS-" + dsList1.getColumn(0, "CONTRACT_DOC_NO"));
        	nIdx = arrColName.indexOf("계약일자");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], this.fnReformatDate(dsList1.getColumn(0, "CONTRACT_DATE"),"1"));
        	nIdx = arrColName.indexOf("계약명");
        	strText = (sPurchaseType == 5) ? " 출장기간 및 금액" : " 투입인력 및 기간";
        	var sContractNm = dsList1.getColumn(0, "CONTRACT_NAME");
        	var strContract = dsList1.getColumn(0, "CLIENT_NAME") + " / " + this.fn_GetReplaceContractNm(dsList1.getColumn(0, "CONTRACT_NAME"));
        	this.dsReceiver.setColumn(0, arrColId[nIdx], strContract);

        	var strTask = dsList1.getColumn(0, "PROJECT_NAME") + " - 개발";
        	nIdx = arrColName.indexOf("수행업무");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], strTask);

        	nIdx = arrColName.indexOf("계약기간");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], this.fnReformatDate(dsList2.getColumn(0, "START_DATE"),"2") +
        			" ~ " + this.fnReformatDate(dsList2.getColumn(0, "END_DATE"),"2"));

        	nIdx = arrColName.indexOf("총금액");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], this.numberWithCommas(dsList3.getSum("TAX_PRICE")));
        	nIdx = arrColName.indexOf("주민번호");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], this.regnoFormat(dsList2.getColumn(0, "REGISTER_NO")));
        	nIdx = arrColName.indexOf("주소");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], dsList5.getColumn(0, "POST_ADDR"));

        	strText = (sPurchaseType == 5) ? " 출장기간 및 금액" : " 투입인력 및 기간";
        	var sContractNm = dsList1.getColumn(0, "CONTRACT_NAME");
        	var strTitle = "" + dsList1.getColumn(0, "CLIENT_NAME") + " "
        						+ this.fn_GetReplaceContractNm(sContractNm) + strText; //협력사

        	nIdx = arrColName.indexOf("지급상세제목");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], strTitle);

        	nIdx = arrColName.indexOf("투입기간");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], this.fnReformatDate(dsList2.getColumn(0, "START_DATE"),"2") +
        			" ~ " + this.fnReformatDate(dsList2.getColumn(0, "END_DATE"),"2"));
        	nIdx = arrColName.indexOf("단가투입월");
        	this.dsReceiver.setColumn(0, arrColId[nIdx],
        			this.numberWithCommas(parseInt(dsList2.getColumn(0, "UNIT_COST")) + parseInt(dsList2.getColumn(0, "EXPENSE"))) +
        			" * " + dsList2.getColumn(0, "MEN_MONTH"));
        	nIdx = arrColName.indexOf("금액");
        	this.dsReceiver.setColumn(0, arrColId[nIdx], this.numberWithCommas(dsList3.getSum("TAX_PRICE")));

        	nIdx = arrColName.indexOf("비고");
        	var remark = (dsList1.getColumn(0,"REMARKS_FLAG")=="1"?" "+dsList2.getColumn(0,"REMARKS"):"");
        	if(this.gfnIsNull(remark)) {
        		this.dsReceiver.setColumn(0, arrColId[nIdx], dsList2.getColumn(0, "MEN_MONTH") + "MM");
        	} else {
        		this.dsReceiver.setColumn(0, arrColId[nIdx], remark);
        	}

        	var sPayTerm = dsList1.getColumn(0, "PAY_TERM");
        	var sId = "";
        	for(var j=0; j<dsList3.rowcount; j++)
        	{
        		sId = (j+1);
        		if(dsList1.getColumn(0, "LUMP_PAY_FLAG") == "1" && j>0)
        			break;
        		if(sPayTerm == "M") {
        			nIdx = arrColName.indexOf("역무기간" + sId);
        			this.dsReceiver.setColumn(0, arrColId[nIdx], this.fnReformatDate(dsList3.getColumn(j, "START_DATE"),"2") +
        					" ~ " + this.fnReformatDate(dsList3.getColumn(j, "END_DATE"),"2"));
        		} else {
        			nIdx = arrColName.indexOf("역무기간" + sId);
        			this.dsReceiver.setColumn(0, arrColId[nIdx], dsList3.getColumn(0, "REMARKS"));
        		}
        		nIdx = arrColName.indexOf("지급일" + sId);
        		this.dsReceiver.setColumn(0, arrColId[nIdx], this.fnReformatDate(dsList3.getColumn(j, "PAY_PLAN_DATE"),"2"));

        		nIdx = arrColName.indexOf("금액" + sId);
        		this.dsReceiver.setColumn(0, arrColId[nIdx], this.numberWithCommas(dsList3.getColumn(j, "TAX_PRICE")));
        	}
        ...
        */
        ...
        this.divBottom_btnPreview_onclick = function(obj,e)
        {
        	this.UbiDocuMaker00.saveDoc(this.dsHistory, this.dsReceiver);
        };
        ...
        */
        this.UbiDocuMaker00_onnotify = function(obj, e)
        {
        	if(e.data) {
        		...
        		} else if(e.data.UTYPE == "SAVEDOC") {
        			if(e.data.STATUS == 1) {	// 성공
        				this.fnOpenView();
        			} else {
        ...
        this.fnOpenView = function()
        {
        	...
        	var oArg    = { pvUUID:		docId,
        					...
        					pvTitle :	Doctitle,
        					pvSeq :	seq,
        					pvUserId:	this.dsReceiver.getColumn(0,  "USER_ID")
        				  };
        ...
        */
        ...
        this.on_initEvent = function()
        {
            ...
            this.divBottom.form.btnPreview.addEventHandler("onclick",this.divBottom_btnPreview_onclick,this);
        ...
        this.loadIncludeScript("projectDocMailPu.xfdl");
