(function()
{
	return function()
	{
		nexacro._setCSSMaps(
		{
            "HScrollBarControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #47484b, 0px none, 0px none, 0px none"),
            						"padding" : nexacro.PaddingObject("1px 1px 1px 1px")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #47484b, 0px none, 0px none, 0px none"),
            						"padding" : nexacro.PaddingObject("1px 1px 1px 1px")
            					},
            					"disabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #2b2c2e, 0px none, 0px none, 0px none")
            					}
            				}
            			},
            			"decbutton" :
            			{
            				"parent" :
            				{
            					"HScrollBarControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_Hdec.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_HdecO.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_HdecO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_HdecO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_HdecO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_HdecD.png\")")
            							}
            						}
            					},
            					"VScrollBarControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_Vdec.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_VdecO.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_VdecO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_VdecO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_VdecO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_VdecD.png\")")
            							}
            						}
            					},
            					"vscrollbar" :
            					{
            						"parent" :
            						{
            							"Grid" :
            							{
            								"class" :
            								[
            									{
            										"grd_LF_Menu" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												}
            											}
            										}
            									},
            									{
            										"grd_MDI_List" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												}
            											}
            										}
            									},
            									{
            										"grd_TF_FavSet" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												}
            											}
            										}
            									},
            									{
            										"grd_MF_Notice" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												}
            											}
            										}
            									},
            									{
            										"grd_WF_CalMgmt" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												}
            											}
            										}
            									}
            								]
            							},
            							"Form" :
            							{
            								"class" :
            								[
            									{
            										"frm_MF_MegaMenu" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												}
            											}
            										}
            									}
            								]
            							},
            							"ListView" :
            							{
            								"class" :
            								[
            									{
            										"lst_WF_Calview" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												}
            											}
            										}
            									}
            								]
            							}
            						}
            					}
            				}
            			},
            			"incbutton" :
            			{
            				"parent" :
            				{
            					"HScrollBarControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_Hinc.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_HincD.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_HincO.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_HincO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_HincO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_HincO.png\")")
            							}
            						}
            					},
            					"VScrollBarControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_Vinc.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_VincO.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_VincO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_VincO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_VincO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_VincD.png\")")
            							}
            						}
            					},
            					"vscrollbar" :
            					{
            						"parent" :
            						{
            							"Grid" :
            							{
            								"class" :
            								[
            									{
            										"grd_LF_Menu" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												}
            											}
            										}
            									},
            									{
            										"grd_MDI_List" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												}
            											}
            										}
            									},
            									{
            										"grd_TF_FavSet" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												}
            											}
            										}
            									},
            									{
            										"grd_MF_Notice" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												}
            											}
            										}
            									},
            									{
            										"grd_WF_CalMgmt" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												}
            											}
            										}
            									}
            								]
            							},
            							"Form" :
            							{
            								"class" :
            								[
            									{
            										"frm_MF_MegaMenu" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_None.png\")")
            												}
            											}
            										}
            									}
            								]
            							},
            							"ListView" :
            							{
            								"class" :
            								[
            									{
            										"lst_WF_Calview" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"mouseover" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"focused" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"pushed" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"selected" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												},
            												"disabled" :
            												{
            													"icon" : nexacro.UrlObject("none")
            												}
            											}
            										}
            									}
            								]
            							}
            						}
            					}
            				}
            			},
            			"trackbar" :
            			{
            				"parent" :
            				{
            					"HScrollBarControl" :
            					{
            						"self" :
            						{
            							"focused" :
            							{
            							},
            							"mouseover" :
            							{
            							},
            							"pushed" :
            							{
            							},
            							"selected" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"VScrollBarControl" :
            					{
            						"self" :
            						{
            							"focused" :
            							{
            							},
            							"mouseover" :
            							{
            							},
            							"pushed" :
            							{
            							},
            							"selected" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"vscrollbar" :
            					{
            						"parent" :
            						{
            							"Grid" :
            							{
            								"class" :
            								[
            									{
            										"grd_LF_Menu" :
            										{
            											"self" :
            											{
            												"focused" :
            												{
            												},
            												"mouseover" :
            												{
            												},
            												"pushed" :
            												{
            												},
            												"selected" :
            												{
            												},
            												"disabled" :
            												{
            												}
            											}
            										}
            									},
            									{
            										"grd_MDI_List" :
            										{
            											"self" :
            											{
            												"focused" :
            												{
            												},
            												"mouseover" :
            												{
            												},
            												"pushed" :
            												{
            												},
            												"selected" :
            												{
            												},
            												"disabled" :
            												{
            												}
            											}
            										}
            									},
            									{
            										"grd_TF_FavSet" :
            										{
            											"self" :
            											{
            												"focused" :
            												{
            												},
            												"mouseover" :
            												{
            												},
            												"pushed" :
            												{
            												},
            												"selected" :
            												{
            												},
            												"disabled" :
            												{
            												}
            											}
            										}
            									},
            									{
            										"grd_MF_Notice" :
            										{
            											"self" :
            											{
            												"focused" :
            												{
            												},
            												"mouseover" :
            												{
            												},
            												"pushed" :
            												{
            												},
            												"selected" :
            												{
            												},
            												"disabled" :
            												{
            												}
            											}
            										}
            									},
            									{
            										"grd_WF_CalMgmt" :
            										{
            											"self" :
            											{
            												"focused" :
            												{
            												},
            												"mouseover" :
            												{
            												},
            												"pushed" :
            												{
            												},
            												"selected" :
            												{
            												},
            												"disabled" :
            												{
            												}
            											}
            										}
            									}
            								]
            							},
            							"Form" :
            							{
            								"class" :
            								[
            									{
            										"frm_MF_MegaMenu" :
            										{
            											"self" :
            											{
            												"focused" :
            												{
            												},
            												"mouseover" :
            												{
            												},
            												"pushed" :
            												{
            												},
            												"selected" :
            												{
            												},
            												"disabled" :
            												{
            												}
            											}
            										}
            									}
            								]
            							},
            							"ListView" :
            							{
            								"class" :
            								[
            									{
            										"lst_WF_Calview" :
            										{
            											"self" :
            											{
            												"focused" :
            												{
            												},
            												"mouseover" :
            												{
            												},
            												"pushed" :
            												{
            												},
            												"selected" :
            												{
            												}
            											}
            										}
            									}
            								]
            							}
            						}
            					}
            				}
            			},
            			"VScrollBarControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("0px none, 0px none, 0px none, 1px solid #47484b"),
            						"padding" : nexacro.PaddingObject("1px 1px 1px 1px")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("0px none, 0px none, 0px none, 1px solid #47484b"),
            						"padding" : nexacro.PaddingObject("1px 1px 1px 1px")
            					},
            					"disabled" :
            					{
            						"border" : nexacro.BorderObject("0px none, 0px none, 0px none, 1px solid #2b2c2e")
            					}
            				}
            			},
            			"Form" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("0px none"),
            						"color" : nexacro.ColorObject("#ececec"),
            						"font" : nexacro.FontObject("13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            					},
            					"contents" :
            					{
            					}
            				},
            				"class" :
            				[
            					{
            						"frm_LF_Bg" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #000000, 0px none, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"frm_POP_Bg" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #23262d")
            								}
            							}
            						}
            					}
            				]
            			},
            			"Button" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #7b7e82"),
            						"padding" : nexacro.PaddingObject("0px 12px 0px 12px"),
            						"color" : nexacro.ColorObject("#ececec"),
            						"letterSpacing" : nexacro.CSSValueObject("-1px")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #eaeaeb")
            					},
            					"pushed" :
            					{
            						"border" : nexacro.BorderObject("1px solid #eaeaeb")
            					},
            					"selected" :
            					{
            						"border" : nexacro.BorderObject("1px solid #eaeaeb")
            					},
            					"disabled" :
            					{
            					}
            				},
            				"class" :
            				[
            					{
            						"btn_TF_Menu" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            									"color" : nexacro.ColorObject("#31bff2"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            									"color" : nexacro.ColorObject("#31bff2"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            									"color" : nexacro.ColorObject("#31bff2"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"btn_TF_MenuS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            									"color" : nexacro.ColorObject("#31bff2"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            									"color" : nexacro.ColorObject("#31bff2"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            									"color" : nexacro.ColorObject("#31bff2"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            									"color" : nexacro.ColorObject("#31bff2"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            									"color" : nexacro.ColorObject("#31bff2"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"btn_TF_Prev" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #565b74"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #565b74"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ffffff")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ffffff")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ffffff")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_TF_Next" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #565b74"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #565b74"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ffffff")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ffffff")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ffffff")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_TF_Logout" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_TF_MegaMenu" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_TF_Logbtn_TF_MegaMenuout" :
            						{
            							"self" :
            							{
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_TF_User" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Close" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								},
            								"mouseover" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								},
            								"pushed" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Open" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								},
            								"mouseover" :
            								{
            									"padding" : nexacro.PaddingObject("0px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								},
            								"pushed" :
            								{
            									"padding" : nexacro.PaddingObject("0px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Home" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_HomeS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_CloseAll" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Max" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Restore" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Vertical" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Horizontal" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_List" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Voc" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Left" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalLeft" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Right" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalRight" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_Menu" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 40px 0px 20px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none"),
            									"color" : nexacro.ColorObject("#90939e"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 40px 0px 20px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none"),
            									"color" : nexacro.ColorObject("#90939e"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #101112, 0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px 40px 0px 20px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"color" : nexacro.ColorObject("#ececec"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"pushed" :
            								{
            									"padding" : nexacro.PaddingObject("0px 40px 0px 20px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"color" : nexacro.ColorObject("#ececec"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_MenuS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 40px 0px 20px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"color" : nexacro.ColorObject("#ececec"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 40px 0px 20px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"color" : nexacro.ColorObject("#ececec"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px 40px 0px 20px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"color" : nexacro.ColorObject("#ececec"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"pushed" :
            								{
            									"padding" : nexacro.PaddingObject("0px 40px 0px 20px"),
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none"),
            									"color" : nexacro.ColorObject("#ececec"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #393a3c, 1px solid #1e1f21, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MDI_TabClose" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_Menu" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_MenuS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_Fav" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_FavS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_MenuClose" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #e1e2e7, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #e1e2e7, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #e1e2e7, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #e1e2e7, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #e1e2e7, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_MoveLeft" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_MoveRight" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_MoveUp" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_MoveDown" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_MenuOpen" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #e1e2e7, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #e1e2e7, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #e1e2e7, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #e1e2e7, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #e1e2e7, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_Sch" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_Edit" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px"),
            									"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px"),
            									"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_Cancel" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px"),
            									"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px"),
            									"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_Save" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #23273c"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #23273c"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_Up" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #393a3c"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #393a3c"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_Down" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #393a3c"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #393a3c"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LF_Grip" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px"),
            									"border" : nexacro.BorderObject("1px solid #74767A"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_LF_Grip.png\")")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #9C9FA6"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_LF_Grip.png\")")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #9C9FA6"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_LF_Grip.png\")")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #5B5D61"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_LF_Grip.png\")")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #5B5D61"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_LF_Grip.png\")")
            								}
            							}
            						}
            					},
            					{
            						"btn_MF_More" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MF_New" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MF_Restore" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MF_Close" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MF_Quick01" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 1px solid #3a3b3d, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 35px 0px"),
            									"color" : nexacro.ColorObject("#e1e1e1"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 1px solid #3a3b3d, 0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 1px solid #3a3b3d, 0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 1px solid #3a3b3d, 0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 1px solid #3a3b3d, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MF_Quick02" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 1px solid #3a3b3d, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 35px 0px"),
            									"color" : nexacro.ColorObject("#e1e1e1"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 1px solid #3a3b3d, 0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 1px solid #3a3b3d, 0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 1px solid #3a3b3d, 0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 1px solid #3a3b3d, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MF_Quick03" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #3a3b3d, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 35px 0px"),
            									"color" : nexacro.ColorObject("#e1e1e1"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #3a3b3d, 0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #3a3b3d, 0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #3a3b3d, 0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #3a3b3d, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MF_Quick04" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 35px 0px"),
            									"color" : nexacro.ColorObject("#e1e1e1"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 0px none, 0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 0px none, 0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 0px none, 0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 0px none, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MF_Quick05" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 0px none, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 35px 0px"),
            									"color" : nexacro.ColorObject("#e1e1e1"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 0px none, 0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 0px none, 0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 0px none, 0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none, 1px solid #3a3b3d, 0px none, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_MF_Quick06" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 35px 0px"),
            									"color" : nexacro.ColorObject("#e1e1e1"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Crud" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CrudSave" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_CrudSave.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_CrudSave.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CrudDelete" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_CrudDelete.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_CrudDelete.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CrudModify" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_CrudModify.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_CrudModify.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CrudAdd" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_CrudAdd.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_CrudAdd.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3073c7")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Custom" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #2e9eca"),
            									"padding" : nexacro.PaddingObject("0px 15px 0px 15px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #2e9eca"),
            									"padding" : nexacro.PaddingObject("0px 15px 0px 15px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #2e9eca")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #2492bd")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #2492bd")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Search" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Re" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Detail" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #d8d8d8"),
            									"padding" : nexacro.PaddingObject("0px 8px 0px 8px"),
            									"color" : nexacro.ColorObject("#666666"),
            									"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #d8d8d8"),
            									"padding" : nexacro.PaddingObject("0px 8px 0px 8px"),
            									"color" : nexacro.ColorObject("#666666"),
            									"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_EdiSch" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_SchSmall" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Module" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #9fa5b8"),
            									"padding" : nexacro.PaddingObject("0px 7px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #9fa5b8"),
            									"padding" : nexacro.PaddingObject("0px 7px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #9fa5b8"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #9fa5b8"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #9fa5b8"),
            									"color" : nexacro.ColorObject("#ffffff")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Add" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Add.png\")"),
            									"iconPosition" : "left",
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 4px"),
            									"padding" : nexacro.PaddingObject("0px 10px 0px 10px")
            								},
            								"focused" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Add.png\")"),
            									"iconPosition" : "left",
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 4px"),
            									"padding" : nexacro.PaddingObject("0px 10px 0px 10px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_AddSm" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Add.png\")"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"focused" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Add.png\")"),
            									"padding" : nexacro.PaddingObject("0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Delete" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Delete.png\")"),
            									"iconPosition" : "left",
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 4px"),
            									"padding" : nexacro.PaddingObject("0px 10px 0px 10px")
            								},
            								"focused" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Delete.png\")"),
            									"iconPosition" : "left",
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 4px"),
            									"padding" : nexacro.PaddingObject("0px 10px 0px 10px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_DeleteSm" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Delete.png\")"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"focused" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Delete.png\")"),
            									"padding" : nexacro.PaddingObject("0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_ExcelUp" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 3px"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_ExcelUp.png\")"),
            									"iconPosition" : "left"
            								},
            								"focused" :
            								{
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 3px"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_ExcelUp.png\")"),
            									"iconPosition" : "left"
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_ExcelUpSm" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_ExcelUp.png\")"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"focused" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_ExcelUp.png\")"),
            									"padding" : nexacro.PaddingObject("0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_ExcelSave" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 3px"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_ExcelSave.png\")"),
            									"iconPosition" : "left"
            								},
            								"focused" :
            								{
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 3px"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_ExcelSave.png\")"),
            									"iconPosition" : "left"
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_ExcelSaveSm" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_ExcelSave.png\")"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"focused" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_ExcelSave.png\")"),
            									"padding" : nexacro.PaddingObject("0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_TempSave" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 3px"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_TempSave.png\")"),
            									"iconPosition" : "left"
            								},
            								"focused" :
            								{
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 3px"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_TempSave.png\")"),
            									"iconPosition" : "left"
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_TempSaveSm" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_TempSave.png\")"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"focused" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_TempSave.png\")"),
            									"padding" : nexacro.PaddingObject("0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Print" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Help" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Full" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Info" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Ex" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_In" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #45484B"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #45484B"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Normal" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Bookmark" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_BookmarkS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Expand" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #101112"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #101112"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #101112"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #101112"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Collapse" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #101112"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #101112"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #101112"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #101112"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Up" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Down" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Left" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Right" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_PageL" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_PageR" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_PageP" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_PageN" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Page" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#666666"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#ee5555"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#ee5555"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#ee5555"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#ee5555"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_PageS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#ee5555"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#ee5555"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#ee5555"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#ee5555"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#ee5555"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal01" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal02" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal03" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal04" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal05" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal06" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal07" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal08" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal09" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal10" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal11" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Cal12" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            								},
            								"pushed" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"selected" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Light" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_Light.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#858eb0")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_Light.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#858eb0")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_LightS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_LightS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_LightS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_LightS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_LightS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_LightS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_LightS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_LightS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Dark" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_Dark.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#858eb0")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_Dark.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#858eb0")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_DarkS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_DarkS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_DarkS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_DarkS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_DarkS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_DarkS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_DarkS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a8afc9"),
            									"icon" : nexacro.UrlObject("URL(\"theme://images/btn_WF_DarkS.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 5px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Tab" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #44464a"),
            									"color" : nexacro.ColorObject("#9EA1AB")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #44464a"),
            									"color" : nexacro.ColorObject("#9EA1AB")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #44464a")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3683e2"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3683e2"),
            									"color" : nexacro.ColorObject("#ffffff")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_TabS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3683e2"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3683e2"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3683e2"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3683e2"),
            									"color" : nexacro.ColorObject("#ffffff")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3683e2"),
            									"color" : nexacro.ColorObject("#ffffff")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Drop" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb"),
            									"font" : nexacro.FontObject("13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_Sch" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"focused" :
            								{
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_PreBtn" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_NxtBtn" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px")
            								}
            							}
            						}
            					},
            					{
            						"btn_LOGIN_Start" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 16px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_LOGIN_Pw" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#666666"),
            									"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#000000")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#000000")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#000000")
            								}
            							}
            						}
            					},
            					{
            						"btn_POP_Close" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalRefresh" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646669"),
            									"icon" : nexacro.UrlObject("url(\"theme://images/dark/btn_WF_CalRefresh.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 2px"),
            									"color" : nexacro.ColorObject("#bdbfc5"),
            									"font" : nexacro.FontObject("14px bold \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646669"),
            									"icon" : nexacro.UrlObject("url(\"theme://images/dark/btn_WF_CalRefresh.png\")"),
            									"textPadding" : nexacro.PaddingObject("0px 0px 0px 2px"),
            									"color" : nexacro.ColorObject("#bdbfc5"),
            									"font" : nexacro.FontObject("14px bold \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #818388")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #818388")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalToday" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646669"),
            									"color" : nexacro.ColorObject("#bdbfc5"),
            									"font" : nexacro.FontObject("14px bold \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646669"),
            									"color" : nexacro.ColorObject("#bdbfc5"),
            									"font" : nexacro.FontObject("14px bold \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #818388")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #818388")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalClose" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646669"),
            									"color" : nexacro.ColorObject("#bdbfc5"),
            									"font" : nexacro.FontObject("14px bold \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646669"),
            									"color" : nexacro.ColorObject("#bdbfc5"),
            									"font" : nexacro.FontObject("14px bold \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #818388")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #818388")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalOpen" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646669"),
            									"color" : nexacro.ColorObject("#bdbfc5"),
            									"font" : nexacro.FontObject("14px bold \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"selected" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646669"),
            									"color" : nexacro.ColorObject("#bdbfc5"),
            									"font" : nexacro.FontObject("14px bold \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #818388")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("1px solid #818388")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalPrev" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px none"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px none"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalNext" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px none"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px none"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalYearPrev" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px none"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px none"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalYearNext" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px none"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"selected" :
            								{
            									"padding" : nexacro.PaddingObject("0px none"),
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"pushed" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalUp" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalUp.png\")"),
            									"border" : nexacro.BorderObject("1px solid #3E4046")
            								},
            								"mouseover" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalUpO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #818388")
            								},
            								"pushed" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalUpO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #818388")
            								},
            								"selected" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalUpO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #646669")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalUpS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalUpO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #646669")
            								},
            								"selected" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalUpO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #646669")
            								},
            								"mouseover" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalUpO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #818388")
            								},
            								"pushed" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalUpO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #818388")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalDown" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalDown.png\")"),
            									"border" : nexacro.BorderObject("1px solid #3E4046")
            								},
            								"mouseover" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalDownO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #818388")
            								},
            								"pushed" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalDownO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #818388")
            								},
            								"selected" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalDownO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #646669")
            								}
            							}
            						}
            					},
            					{
            						"btn_WF_CalDownS" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalDownO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #646669")
            								},
            								"selected" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalDownO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #646669")
            								},
            								"mouseover" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalDownO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #818388")
            								},
            								"pushed" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_CalDownO.png\")"),
            									"border" : nexacro.BorderObject("1px solid #818388")
            								}
            							}
            						}
            					}
            				]
            			},
            			"Calendar" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"disabled" :
            					{
            					}
            				},
            				"class" :
            				[
            					{
            						"essential" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #f6e5bf"),
            									"color" : nexacro.ColorObject("#333333")
            								},
            								"mouseover" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"readonly" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"error" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ff2d4b")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ff2d4b")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ff2d4b")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ff2d4b")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ff2d4b")
            								}
            							}
            						}
            					},
            					{
            						"cal_MF_Cal" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					}
            				]
            			},
            			"CalendarControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"disabled" :
            					{
            					}
            				}
            			},
            			"calendaredit" :
            			{
            				"parent" :
            				{
            					"Calendar" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 8px")
            							},
            							"mouseover" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 8px")
            							},
            							"focused" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 8px")
            							},
            							"readonly" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 8px")
            							},
            							"disabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 8px")
            							},
            							"nulltext" :
            							{
            								"color" : nexacro.ColorObject("#aeaeae")
            							}
            						}
            					},
            					"CalendarControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 8px")
            							},
            							"mouseover" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 8px")
            							},
            							"focused" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 8px")
            							},
            							"readonly" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 8px")
            							},
            							"disabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 8px")
            							},
            							"nulltext" :
            							{
            								"color" : nexacro.ColorObject("#aeaeae")
            							}
            						}
            					}
            				}
            			},
            			"dropbutton" :
            			{
            				"parent" :
            				{
            					"Calendar" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cal_WF_DropBtn.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cal_WF_DropBtn.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cal_WF_DropBtnO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cal_WF_DropBtnO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cal_WF_DropBtnO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cal_WF_DropBtnD.png\")")
            							}
            						},
            						"class" :
            						[
            							{
            								"essential" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"icon" : nexacro.UrlObject("URL(\"theme://images/cal_WF_DropBtnO.png\")")
            										}
            									}
            								}
            							}
            						]
            					},
            					"CalendarControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cal_WF_DropBtn.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cal_WF_DropBtn.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cal_WF_DropBtnO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cal_WF_DropBtnO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cal_WF_DropBtnO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cal_WF_DropBtnD.png\")")
            							}
            						}
            					},
            					"Combo" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_Drop.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cbo_WF_DropO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cbo_WF_DropO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cbo_WF_DropO.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cbo_WF_DropO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_DropD.png\")")
            							}
            						},
            						"class" :
            						[
            							{
            								"essential" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"icon" : nexacro.UrlObject("URL(\"theme://images/cbo_WF_DropO.png\")")
            										}
            									}
            								}
            							}
            						]
            					},
            					"ComboControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_Drop.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cbo_WF_DropO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cbo_WF_DropO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cbo_WF_DropO.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/cbo_WF_DropO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_DropD.png\")")
            							}
            						}
            					},
            					"MultiCombo" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_Drop.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_DropO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_DropO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_DropO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_DropD.png\")")
            							}
            						}
            					},
            					"MultiComboControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_Drop.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_DropO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_DropO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_DropO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cbo_WF_DropD.png\")")
            							}
            						}
            					}
            				}
            			},
            			"calendarspindownbutton" :
            			{
            				"parent" :
            				{
            					"Calendar" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/spn_WF_DownBtn.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/spn_WF_DownBtn.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/spn_WF_DownBtnD.png\")")
            							}
            						}
            					}
            				}
            			},
            			"calendarspinupbutton" :
            			{
            				"parent" :
            				{
            					"Calendar" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/spn_WF_UpBtn.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/spn_WF_UpBtn.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/spn_WF_UpBtnD.png\")")
            							}
            						}
            					}
            				}
            			},
            			"CheckBox" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_box.png\")"),
            						"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            					},
            					"focused" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_box.png\")"),
            						"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            					},
            					"readonly" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_box.png\")"),
            						"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            					},
            					"mouseover" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxO.png\")")
            					},
            					"mouseover_selected" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxS.png\")")
            					},
            					"selected" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxS.png\")")
            					},
            					"disabled" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxD.png\")")
            					},
            					"disabled_selected" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxDS.png\")")
            					}
            				},
            				"class" :
            				[
            					{
            						"essential" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/chk_WF_boxES.png\")")
            								},
            								"focused" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/chk_WF_boxES.png\")")
            								},
            								"readonly" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/chk_WF_boxES.png\")")
            								},
            								"mouseover" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/chk_WF_boxES.png\")")
            								},
            								"mouseover_selected" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/chk_WF_boxSES.png\")")
            								},
            								"selected" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/chk_WF_boxSES.png\")")
            								}
            							}
            						}
            					},
            					{
            						"chk_LOGIN_Id" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#666666")
            								}
            							}
            						}
            					}
            				]
            			},
            			"CheckBoxControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_box.png\")"),
            						"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            					},
            					"focused" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_box.png\")"),
            						"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            					},
            					"readonly" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_box.png\")"),
            						"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            					},
            					"mouseover" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxO.png\")")
            					},
            					"mouseover_selected" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxS.png\")")
            					},
            					"selected" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxS.png\")")
            					},
            					"disabled" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxD.png\")")
            					},
            					"disabled_selected" :
            					{
            						"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxDS.png\")")
            					}
            				},
            				"class" :
            				[
            					{
            						"essential" :
            						{
            							"self" :
            							{
            								"mouseover" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/chk_WF_boxES.png\")")
            								},
            								"mouseover_selected" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/chk_WF_boxSES.png\")")
            								},
            								"selected" :
            								{
            									"icon" : nexacro.UrlObject("URL(\"theme://images/chk_WF_boxSES.png\")")
            								}
            							}
            						}
            					}
            				]
            			},
            			"checkboxsetitem" :
            			{
            				"parent" :
            				{
            					"CheckBoxSet" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_box.png\")"),
            								"padding" : nexacro.PaddingObject("4px 0px 5px 0px"),
            								"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_box.png\")"),
            								"padding" : nexacro.PaddingObject("4px 0px 5px 0px"),
            								"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxO.png\")")
            							},
            							"mouseover_selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxS.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxS.png\")")
            							},
            							"readonly" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxD.png\")"),
            								"color" : nexacro.ColorObject("#6b6f75")
            							},
            							"readonly_selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark//chk_WF_boxDS.png\")"),
            								"color" : nexacro.ColorObject("#6b6f75")
            							}
            						}
            					},
            					"CheckBoxSetControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_box.png\")"),
            								"padding" : nexacro.PaddingObject("4px 0px 5px 0px"),
            								"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_box.png\")"),
            								"padding" : nexacro.PaddingObject("4px 0px 5px 0px"),
            								"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxO.png\")")
            							},
            							"mouseover_selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxS.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxS.png\")")
            							},
            							"readonly" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/chk_WF_boxD.png\")"),
            								"color" : nexacro.ColorObject("#6b6f75")
            							},
            							"readonly_selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark//chk_WF_boxDS.png\")"),
            								"color" : nexacro.ColorObject("#6b6f75")
            							}
            						}
            					}
            				}
            			},
            			"Combo" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"disabled" :
            					{
            					}
            				},
            				"class" :
            				[
            					{
            						"essential" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #f6e5bf"),
            									"color" : nexacro.ColorObject("#333333")
            								},
            								"mouseover" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"readonly" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"error" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ff2d4b")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ff2d4b")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ff2d4b")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ff2d4b")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ff2d4b")
            								}
            							}
            						}
            					},
            					{
            						"cbo_WF_Cal" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#333333")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#333333")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#333333")
            								}
            							}
            						}
            					},
            					{
            						"cbo_LOGIN_Input" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#000000"),
            									"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#000000"),
            									"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#000000"),
            									"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					}
            				]
            			},
            			"ComboControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"disabled" :
            					{
            					}
            				}
            			},
            			"comboedit" :
            			{
            				"parent" :
            				{
            					"Combo" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 8px")
            							},
            							"focused" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 8px")
            							},
            							"mouseover" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 8px")
            							},
            							"readonly" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 8px")
            							},
            							"disabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"color" : nexacro.ColorObject("#aeaeae")
            							},
            							"nulltext" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"color" : nexacro.ColorObject("#aeaeae")
            							}
            						},
            						"class" :
            						[
            							{
            								"cbo_WF_Cal" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            										}
            									}
            								}
            							},
            							{
            								"cbo_LOGIN_Input" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            										}
            									}
            								}
            							}
            						]
            					},
            					"ComboControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 8px")
            							},
            							"focused" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 8px")
            							},
            							"mouseover" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 8px")
            							},
            							"readonly" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 8px")
            							},
            							"disabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"color" : nexacro.ColorObject("#aeaeae")
            							},
            							"nulltext" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"color" : nexacro.ColorObject("#aeaeae")
            							}
            						}
            					}
            				}
            			},
            			"combolist" :
            			{
            				"parent" :
            				{
            					"Combo" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("1px solid #646982")
            							}
            						}
            					},
            					"ComboControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("1px solid #646982")
            							}
            						}
            					}
            				}
            			},
            			"listboxitem" :
            			{
            				"parent" :
            				{
            					"combolist" :
            					{
            						"parent" :
            						{
            							"Combo" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"padding" : nexacro.PaddingObject("4px 0px 4px 8px"),
            										"color" : nexacro.ColorObject("#333333")
            									},
            									"mouseover" :
            									{
            									},
            									"selected" :
            									{
            									}
            								}
            							},
            							"ComboControl" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"padding" : nexacro.PaddingObject("4px 0px 4px 8px"),
            										"color" : nexacro.ColorObject("#333333")
            									},
            									"mouseover" :
            									{
            									},
            									"selected" :
            									{
            									}
            								}
            							}
            						}
            					},
            					"ListBox" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"padding" : nexacro.PaddingObject("4px 0px 4px 10px")
            							},
            							"readonly" :
            							{
            								"padding" : nexacro.PaddingObject("4px 0px 4px 10px")
            							},
            							"focused" :
            							{
            								"padding" : nexacro.PaddingObject("4px 0px 4px 10px")
            							},
            							"mouseover" :
            							{
            							},
            							"selected" :
            							{
            								"color" : nexacro.ColorObject("#333333")
            							},
            							"disabled" :
            							{
            								"color" : nexacro.ColorObject("#575a5e")
            							}
            						}
            					}
            				}
            			},
            			"MultiCombo" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"disabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					}
            				}
            			},
            			"MultiComboControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"disabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					}
            				}
            			},
            			"multicombolist" :
            			{
            				"parent" :
            				{
            					"MultiCombo" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("1px solid #4b4d51")
            							}
            						}
            					},
            					"MultiComboControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("1px solid #4b4d51")
            							}
            						}
            					}
            				}
            			},
            			"checkboxset" :
            			{
            				"parent" :
            				{
            					"multicombolist" :
            					{
            						"parent" :
            						{
            							"MultiCombo" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"padding" : nexacro.PaddingObject("0px 0px 4px 8px"),
            										"color" : nexacro.ColorObject("#ececec")
            									},
            									"readonly" :
            									{
            										"color" : nexacro.ColorObject("#575a5e")
            									}
            								}
            							},
            							"MultiComboControl" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"padding" : nexacro.PaddingObject("0px 0px 4px 8px"),
            										"color" : nexacro.ColorObject("#ececec")
            									},
            									"readonly" :
            									{
            										"color" : nexacro.ColorObject("#575a5e")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"selectallcheckbox" :
            			{
            				"parent" :
            				{
            					"multicombolist" :
            					{
            						"parent" :
            						{
            							"MultiCombo" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #292A2D, 0px none"),
            										"padding" : nexacro.PaddingObject("2px 0px 4px 8px"),
            										"textPadding" : nexacro.PaddingObject("0px 0px 2px 6px"),
            										"color" : nexacro.ColorObject("#ececec")
            									},
            									"mouseover" :
            									{
            										"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #292A2D, 0px none"),
            										"padding" : nexacro.PaddingObject("2px 0px 4px 8px"),
            										"textPadding" : nexacro.PaddingObject("0px 0px 2px 6px"),
            										"color" : nexacro.ColorObject("#ececec")
            									},
            									"selected" :
            									{
            										"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #292A2D, 0px none"),
            										"padding" : nexacro.PaddingObject("2px 0px 4px 8px"),
            										"textPadding" : nexacro.PaddingObject("0px 0px 2px 6px"),
            										"color" : nexacro.ColorObject("#ececec")
            									},
            									"focused" :
            									{
            										"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #292A2D, 0px none"),
            										"padding" : nexacro.PaddingObject("2px 0px 4px 8px"),
            										"textPadding" : nexacro.PaddingObject("0px 0px 2px 6px"),
            										"color" : nexacro.ColorObject("#ececec")
            									}
            								}
            							},
            							"MultiComboControl" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #292A2D, 0px none"),
            										"padding" : nexacro.PaddingObject("2px 0px 4px 8px"),
            										"textPadding" : nexacro.PaddingObject("0px 0px 2px 6px"),
            										"color" : nexacro.ColorObject("#ececec")
            									},
            									"mouseover" :
            									{
            										"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #292A2D, 0px none"),
            										"padding" : nexacro.PaddingObject("2px 0px 4px 8px"),
            										"textPadding" : nexacro.PaddingObject("0px 0px 2px 6px"),
            										"color" : nexacro.ColorObject("#ececec")
            									},
            									"selected" :
            									{
            										"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #292A2D, 0px none"),
            										"padding" : nexacro.PaddingObject("2px 0px 4px 8px"),
            										"textPadding" : nexacro.PaddingObject("0px 0px 2px 6px"),
            										"color" : nexacro.ColorObject("#ececec")
            									},
            									"focused" :
            									{
            										"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #292A2D, 0px none"),
            										"padding" : nexacro.PaddingObject("2px 0px 4px 8px"),
            										"textPadding" : nexacro.PaddingObject("0px 0px 2px 6px"),
            										"color" : nexacro.ColorObject("#ececec")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"multicombotagbox" :
            			{
            				"parent" :
            				{
            					"MultiCombo" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"padding" : nexacro.PaddingObject("5px 8px 5px 8px"),
            								"color" : nexacro.ColorObject("#ececec")
            							}
            						}
            					},
            					"MultiComboControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"padding" : nexacro.PaddingObject("5px 8px 5px 8px"),
            								"color" : nexacro.ColorObject("#ececec")
            							}
            						}
            					}
            				}
            			},
            			"tagboxedit" :
            			{
            				"parent" :
            				{
            					"multicombotagbox" :
            					{
            						"parent" :
            						{
            							"MultiCombo" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"border" : nexacro.BorderObject("1px solid #4b4d51"),
            										"padding" : nexacro.PaddingObject("0px 5px 0px 5px")
            									},
            									"mouseover" :
            									{
            										"border" : nexacro.BorderObject("1px solid #313340")
            									},
            									"focused" :
            									{
            										"border" : nexacro.BorderObject("1px solid #313340")
            									}
            								}
            							},
            							"MultiComboControl" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"border" : nexacro.BorderObject("1px solid #4b4d51"),
            										"padding" : nexacro.PaddingObject("0px 5px 0px 5px")
            									},
            									"mouseover" :
            									{
            										"border" : nexacro.BorderObject("1px solid #313340")
            									},
            									"focused" :
            									{
            										"border" : nexacro.BorderObject("1px solid #313340")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"TagBoxControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"padding" : nexacro.PaddingObject("3px 10px 2px 0px")
            					},
            					"mouseover" :
            					{
            						"padding" : nexacro.PaddingObject("3px 10px 2px 0px")
            					},
            					"disabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					}
            				}
            			},
            			"tagbutton" :
            			{
            				"parent" :
            				{
            					"TagBoxControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/tab_WF_X.png\")"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 3px")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/tab_WF_X.png\")"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 3px")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/tab_WF_X.png\")"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 3px")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/tab_WF_X.png\")"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 3px")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/tab_WF_X.png\")"),
            								"padding" : nexacro.PaddingObject("0px 0px 0px 3px")
            							}
            						}
            					}
            				}
            			},
            			"tagtext" :
            			{
            				"parent" :
            				{
            					"TagBoxControl" :
            					{
            						"self" :
            						{
            							"disabled" :
            							{
            								"color" : nexacro.ColorObject("#aeaeae")
            							}
            						}
            					}
            				}
            			},
            			"Edit" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51"),
            						"padding" : nexacro.PaddingObject("0px 0px 0px 8px")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"nulltext" :
            					{
            						"color" : nexacro.ColorObject("#aeaeae")
            					},
            					"disabled" :
            					{
            					}
            				},
            				"class" :
            				[
            					{
            						"essential" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #f6e5bf"),
            									"color" : nexacro.ColorObject("#333333")
            								},
            								"mouseover" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"readonly" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"essentialSub" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #f6e5bf"),
            									"color" : nexacro.ColorObject("#333333")
            								}
            							}
            						}
            					},
            					{
            						"edt_WF_SearchEssential" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #f6e5bf"),
            									"padding" : nexacro.PaddingObject("0px 30px 0px 8px"),
            									"color" : nexacro.ColorObject("#333333")
            								},
            								"mouseover" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"readonly" :
            								{
            								},
            								"nulltext" :
            								{
            									"color" : nexacro.ColorObject("#aaaaaa")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #dddddd"),
            									"color" : nexacro.ColorObject("#aaaaaa")
            								}
            							}
            						}
            					},
            					{
            						"output" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								}
            							}
            						}
            					},
            					{
            						"error" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"nulltext" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								}
            							}
            						}
            					},
            					{
            						"edi_LF_Sch" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 55px 0px 17px"),
            									"color" : nexacro.ColorObject("#ebebeb"),
            									"font" : nexacro.FontObject("14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 55px 0px 17px"),
            									"color" : nexacro.ColorObject("#ebebeb"),
            									"font" : nexacro.FontObject("14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 55px 0px 17px"),
            									"color" : nexacro.ColorObject("#ebebeb"),
            									"font" : nexacro.FontObject("14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 55px 0px 17px"),
            									"color" : nexacro.ColorObject("#ebebeb"),
            									"font" : nexacro.FontObject("14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								},
            								"nulltext" :
            								{
            									"color" : nexacro.ColorObject("#73747c")
            								}
            							}
            						}
            					},
            					{
            						"edt_WF_Search" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #4b4d51"),
            									"padding" : nexacro.PaddingObject("0px 30px 0px 8px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646982")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646982")
            								},
            								"readonly" :
            								{
            								},
            								"nulltext" :
            								{
            									"color" : nexacro.ColorObject("#aaaaaa")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #dddddd"),
            									"color" : nexacro.ColorObject("#aaaaaa")
            								}
            							}
            						}
            					},
            					{
            						"edt_WF_Cbo" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #dddddd"),
            									"padding" : nexacro.PaddingObject("0px 28px 0px 8px")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("1px solid #dddddd"),
            									"padding" : nexacro.PaddingObject("0px 28px 0px 8px")
            								}
            							}
            						}
            					},
            					{
            						"edt_WF_Read" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #dddddd"),
            									"padding" : nexacro.PaddingObject("0px 28px 0px 8px")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("1px solid #dddddd"),
            									"padding" : nexacro.PaddingObject("0px 28px 0px 8px")
            								}
            							}
            						}
            					},
            					{
            						"edt_LOGIN_Input" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#000000"),
            									"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"letterSpacing" : nexacro.CSSValueObject("1px")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#000000"),
            									"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"letterSpacing" : nexacro.CSSValueObject("1px")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            									"color" : nexacro.ColorObject("#000000"),
            									"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"letterSpacing" : nexacro.CSSValueObject("1px")
            								}
            							}
            						}
            					}
            				]
            			},
            			"EditControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51"),
            						"padding" : nexacro.PaddingObject("0px 0px 0px 8px")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"nulltext" :
            					{
            						"color" : nexacro.ColorObject("#aeaeae")
            					},
            					"disabled" :
            					{
            					}
            				}
            			},
            			"FileDownload" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #7b7e82"),
            						"padding" : nexacro.PaddingObject("0px 12px 0px 12px"),
            						"color" : nexacro.ColorObject("#ececec"),
            						"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #eaeaeb")
            					},
            					"pushed" :
            					{
            						"border" : nexacro.BorderObject("1px solid #eaeaeb")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #eaeaeb")
            					},
            					"disabled" :
            					{
            					}
            				}
            			},
            			"FileUpload" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"selected" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"disabled" :
            					{
            					}
            				}
            			},
            			"fileuploaditem" :
            			{
            				"parent" :
            				{
            					"FileUpload" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("0px none")
            							},
            							"focused" :
            							{
            								"border" : nexacro.BorderObject("0px none")
            							}
            						}
            					}
            				}
            			},
            			"fileuploaditemedit" :
            			{
            				"parent" :
            				{
            					"fileuploaditem" :
            					{
            						"parent" :
            						{
            							"FileUpload" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"border" : nexacro.BorderObject("1px solid #4b4d51")
            									},
            									"mouseover" :
            									{
            										"border" : nexacro.BorderObject("1px solid #ffffff")
            									},
            									"focused" :
            									{
            										"border" : nexacro.BorderObject("1px solid #ffffff")
            									},
            									"readonly" :
            									{
            										"border" : nexacro.BorderObject("1px solid #ffffff")
            									},
            									"nulltext" :
            									{
            										"color" : nexacro.ColorObject("#aeaeae")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"fileuploaditembutton" :
            			{
            				"parent" :
            				{
            					"fileuploaditem" :
            					{
            						"parent" :
            						{
            							"FileUpload" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"border" : nexacro.BorderObject("1px solid #7b7e82"),
            										"padding" : nexacro.PaddingObject("0px 12px 0px 12px"),
            										"color" : nexacro.ColorObject("#ececec"),
            										"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            									},
            									"mouseover" :
            									{
            										"border" : nexacro.BorderObject("1px solid #eaeaeb")
            									},
            									"pushed" :
            									{
            										"border" : nexacro.BorderObject("1px solid #eaeaeb")
            									},
            									"selected" :
            									{
            										"border" : nexacro.BorderObject("1px solid #eaeaeb")
            									},
            									"focused" :
            									{
            										"border" : nexacro.BorderObject("1px solid #eaeaeb")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"Grid" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #47484b, 1px solid #393a3c, 1px solid #101112, 1px solid #393a3c")
            					}
            				},
            				"class" :
            				[
            					{
            						"grd_TF_FavSet" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"grd_MDI_List" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"grd_LF_Menu" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"grd_MF_Notice" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"grd_MF_Todo" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#ffffff")
            								}
            							}
            						}
            					},
            					{
            						"grd_MF_MegaMenu" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"grd_WF_Comment" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a0a4b0, 0px none, 0px none, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"grd_WF_List" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"grd_WF_Cbo" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#333333")
            								}
            							}
            						}
            					},
            					{
            						"grd_WF_Cal" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"color" : nexacro.ColorObject("#333333")
            								}
            							}
            						}
            					},
            					{
            						"grd_WF_CalHead" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"grd_WF_CalMgmt" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					}
            				]
            			},
            			"cell" :
            			{
            				"parent" :
            				{
            					"row" :
            					{
            						"parent" :
            						{
            							"head" :
            							{
            								"parent" :
            								{
            									"Grid" :
            									{
            										"self" :
            										{
            											"enabled" :
            											{
            												"border" : nexacro.BorderObject("1px solid #47484b"),
            												"color" : nexacro.ColorObject("#ececec"),
            												"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            											}
            										},
            										"class" :
            										[
            											{
            												"grd_MF_MegaMenu" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"padding" : nexacro.PaddingObject("0px 8px"),
            															"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\""),
            															"color" : nexacro.ColorObject("#ffffff")
            														}
            													}
            												}
            											},
            											{
            												"grd_WF_Cal" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"color" : nexacro.ColorObject("#333333")
            														}
            													}
            												}
            											},
            											{
            												"grd_WF_CalHead" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none")
            														}
            													}
            												}
            											},
            											{
            												"grd_WF_CalMgmt" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"color" : nexacro.ColorObject("#a4a7b8"),
            															"font" : nexacro.FontObject("14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            														}
            													}
            												}
            											}
            										]
            									}
            								}
            							},
            							"body" :
            							{
            								"parent" :
            								{
            									"Grid" :
            									{
            										"self" :
            										{
            											"enabled" :
            											{
            												"border" : nexacro.BorderObject("1px solid #393a3c"),
            												"padding" : nexacro.PaddingObject("4px 4px 4px 4px")
            											},
            											"mouseover" :
            											{
            											},
            											"selected" :
            											{
            											}
            										},
            										"class" :
            										[
            											{
            												"grd_TF_FavSet" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none")
            														},
            														"mouseover" :
            														{
            															"border" : nexacro.BorderObject("0px none")
            														},
            														"selected" :
            														{
            															"border" : nexacro.BorderObject("0px none")
            														}
            													}
            												}
            											},
            											{
            												"grd_MDI_List" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"padding" : nexacro.PaddingObject("0px 10px 0px 28px")
            														},
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            															"color" : nexacro.ColorObject("#ffffff")
            														}
            													}
            												}
            											},
            											{
            												"grd_LF_Menu" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"color" : nexacro.ColorObject("#ebebeb"),
            															"font" : nexacro.FontObject("13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            														},
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            															"color" : nexacro.ColorObject("#4699ff")
            														}
            													}
            												}
            											},
            											{
            												"grd_MF_Notice" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none")
            														},
            														"selected" :
            														{
            															"border" : nexacro.BorderObject("0px none")
            														},
            														"mouseover" :
            														{
            														}
            													}
            												}
            											},
            											{
            												"grd_MF_Todo" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"color" : nexacro.ColorObject("#ffffff")
            														},
            														"selected" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"color" : nexacro.ColorObject("#ffffff")
            														},
            														"mouseover" :
            														{
            														}
            													}
            												}
            											},
            											{
            												"grd_MF_MegaMenu" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"padding" : nexacro.PaddingObject("0px 8px"),
            															"color" : nexacro.ColorObject("#BDBEBE")
            														},
            														"focused" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"padding" : nexacro.PaddingObject("0px 8px"),
            															"color" : nexacro.ColorObject("#BDBEBE")
            														},
            														"mouseover" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"padding" : nexacro.PaddingObject("0px 8px"),
            															"color" : nexacro.ColorObject("#BDBEBE")
            														},
            														"selected" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"padding" : nexacro.PaddingObject("0px 8px"),
            															"color" : nexacro.ColorObject("#BDBEBE")
            														}
            													}
            												}
            											},
            											{
            												"grd_WF_NoneS" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											},
            											{
            												"grd_WF_Comment" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"wordWrap" : "char"
            														},
            														"focused" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"wordWrap" : "char"
            														},
            														"mouseover" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"wordWrap" : "char"
            														},
            														"selected" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"wordWrap" : "char"
            														}
            													}
            												}
            											},
            											{
            												"grd_WF_List" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #a0a4b0, 0px none"),
            															"padding" : nexacro.PaddingObject("0px 0px 0px 20px")
            														},
            														"focused" :
            														{
            															"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #a0a4b0, 0px none"),
            															"padding" : nexacro.PaddingObject("0px 0px 0px 20px")
            														},
            														"mouseover" :
            														{
            															"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #a0a4b0, 0px none")
            														},
            														"selected" :
            														{
            															"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #a0a4b0, 0px none")
            														}
            													}
            												}
            											},
            											{
            												"grd_WF_Cbo" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"padding" : nexacro.PaddingObject("0px 0px 0px 8px"),
            															"color" : nexacro.ColorObject("#333333")
            														}
            													}
            												}
            											},
            											{
            												"grd_WF_Cal" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"color" : nexacro.ColorObject("#333333")
            														},
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            															"color" : nexacro.ColorObject("#ffffff"),
            															"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            														}
            													}
            												}
            											},
            											{
            												"grd_WF_CalMgmt" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none, 1px solid #37383a, 0px none, 1px solid #37383a"),
            															"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            															"color" : nexacro.ColorObject("#cccbd6")
            														},
            														"mouseover" :
            														{
            															"border" : nexacro.BorderObject("0px none, 1px solid #37383a, 0px none, 1px solid #37383a"),
            															"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            															"color" : nexacro.ColorObject("#cccbd6")
            														},
            														"selected" :
            														{
            															"border" : nexacro.BorderObject("0px none, 1px solid #37383a, 0px none, 1px solid #37383a"),
            															"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            															"color" : nexacro.ColorObject("#cccbd6")
            														}
            													}
            												}
            											}
            										]
            									}
            								}
            							},
            							"summary" :
            							{
            								"parent" :
            								{
            									"Grid" :
            									{
            										"self" :
            										{
            											"enabled" :
            											{
            												"border" : nexacro.BorderObject("1px solid #393a3c"),
            												"padding" : nexacro.PaddingObject("4px 10px 4px 10px"),
            												"color" : nexacro.ColorObject("#ffffff"),
            												"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					"body" :
            					{
            						"parent" :
            						{
            							"ListView" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"border" : nexacro.BorderObject("0px none")
            									},
            									"mouseover" :
            									{
            										"border" : nexacro.BorderObject("0px none")
            									},
            									"focused" :
            									{
            										"border" : nexacro.BorderObject("0px none")
            									},
            									"disabled" :
            									{
            										"border" : nexacro.BorderObject("0px none")
            									},
            									"readonly" :
            									{
            										"border" : nexacro.BorderObject("0px none")
            									}
            								},
            								"class" :
            								[
            									{
            										"lst_WF_Calview" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"border" : nexacro.BorderObject("0px none"),
            													"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            													"color" : nexacro.ColorObject("#94959b")
            												},
            												"mouseover" :
            												{
            													"border" : nexacro.BorderObject("0px none"),
            													"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            													"color" : nexacro.ColorObject("#94959b")
            												},
            												"focused" :
            												{
            													"border" : nexacro.BorderObject("0px none"),
            													"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            													"color" : nexacro.ColorObject("#94959b")
            												},
            												"disabled" :
            												{
            													"border" : nexacro.BorderObject("0px none"),
            													"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            													"color" : nexacro.ColorObject("#94959b")
            												},
            												"readonly" :
            												{
            													"border" : nexacro.BorderObject("0px none"),
            													"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            													"color" : nexacro.ColorObject("#94959b")
            												}
            											}
            										}
            									}
            								]
            							}
            						}
            					}
            				},
            				"class" :
            				[
            					{
            						"essential" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"head" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"focused" :
            														{
            														},
            														"readonly" :
            														{
            														},
            														"selected" :
            														{
            														},
            														"blinked" :
            														{
            														},
            														"disabled" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Fixing" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"head" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("1px solid #47484b")
            														}
            													}
            												}
            											}
            										},
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none, 1px solid #47484b, 1px solid #393a3c, 0px none")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_FilterNo" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"head" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"padding" : nexacro.PaddingObject("0px 0px 0px 18px")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_FilterS" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"head" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"padding" : nexacro.PaddingObject("0px 0px 0px 18px")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Point01" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"color" : nexacro.ColorObject("#e10000")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Point02" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"color" : nexacro.ColorObject("#f28005")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Point03" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"color" : nexacro.ColorObject("#2756b6")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Point04" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"color" : nexacro.ColorObject("#2d9d0f")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Link" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"color" : nexacro.ColorObject("#228ff5")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Add" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Modify" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Delete" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_DotRed" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_DotGreen" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_PointTxt01" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_PointTxt02" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_PointTxt03" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_PointTxt04" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Chk" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"focused" :
            														{
            														},
            														"mouseover" :
            														{
            														},
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Sum01" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"summary" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("1px solid #eeeeee"),
            															"color" : nexacro.ColorObject("#333333"),
            															"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Sum02" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"summary" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("1px solid #eeeeee"),
            															"color" : nexacro.ColorObject("#333333"),
            															"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Sum03" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"summary" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("1px solid #eeeeee"),
            															"color" : nexacro.ColorObject("#333333"),
            															"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_DetailTitle" :
            						{
            							"parent" :
            							{
            								"detail" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"padding" : nexacro.PaddingObject("0px 0px 0px 10px"),
            													"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_LF_Lev" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_LF_Menu" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("15px 0px 5px 0px"),
            																		"color" : nexacro.ColorObject("#ebebeb"),
            																		"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_DecoNo" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"mouseover" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"label01" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_Todo" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"focused" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"readonly" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	}
            																}
            															}
            														},
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								},
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"label02" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_Todo" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"focused" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"readonly" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	}
            																}
            															}
            														},
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								},
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"label03" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_Todo" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"focused" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"readonly" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	}
            																}
            															}
            														},
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								},
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"label04" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_Todo" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"focused" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"readonly" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	}
            																}
            															}
            														},
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								},
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"label05" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_Todo" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"focused" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"readonly" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	}
            																}
            															}
            														},
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								},
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"label06" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_Todo" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"focused" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"readonly" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	}
            																}
            															}
            														},
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								},
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"label07" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_Todo" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"focused" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"readonly" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	}
            																}
            															}
            														},
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								},
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"label08" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_Todo" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"focused" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"readonly" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	}
            																}
            															}
            														},
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								},
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"label09" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_Todo" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"focused" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"readonly" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	}
            																}
            															}
            														},
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								},
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"label10" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_Todo" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"focused" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"readonly" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px")
            																	}
            																}
            															}
            														},
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            																		"color" : nexacro.ColorObject("#333333")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								},
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("8px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"color" : nexacro.ColorObject("#FFFFFF"),
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Dp04" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_MF_MegaMenu" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 30px"),
            																		"color" : nexacro.ColorObject("#BDBEBE")
            																	},
            																	"focused" :
            																	{
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 30px"),
            																		"color" : nexacro.ColorObject("#BDBEBE")
            																	},
            																	"mouseover" :
            																	{
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 30px"),
            																		"color" : nexacro.ColorObject("#BDBEBE")
            																	},
            																	"selected" :
            																	{
            																		"border" : nexacro.BorderObject("0px none"),
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 30px"),
            																		"color" : nexacro.ColorObject("#BDBEBE")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Line" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #e1e2e7, 0px none"),
            															"color" : nexacro.ColorObject("#999999"),
            															"font" : nexacro.FontObject("11px \"Tahoma\"")
            														},
            														"focused" :
            														{
            															"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #e1e2e7, 0px none"),
            															"color" : nexacro.ColorObject("#999999"),
            															"font" : nexacro.FontObject("11px \"Tahoma\"")
            														},
            														"mouseover" :
            														{
            															"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #e1e2e7, 0px none"),
            															"color" : nexacro.ColorObject("#999999"),
            															"font" : nexacro.FontObject("11px \"Tahoma\"")
            														},
            														"selected" :
            														{
            															"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #e1e2e7, 0px none"),
            															"color" : nexacro.ColorObject("#999999"),
            															"font" : nexacro.FontObject("11px \"Tahoma\"")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_Id" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"color" : nexacro.ColorObject("#000000"),
            															"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            														},
            														"focused" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"color" : nexacro.ColorObject("#000000"),
            															"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            														},
            														"mouseover" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"color" : nexacro.ColorObject("#000000"),
            															"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            														},
            														"selected" :
            														{
            															"border" : nexacro.BorderObject("0px none"),
            															"color" : nexacro.ColorObject("#000000"),
            															"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"saturday" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"head" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_Cal" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"color" : nexacro.ColorObject("#239cf0")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										},
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_Cal" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"color" : nexacro.ColorObject("#239cf0")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"sunday" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"head" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_Cal" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"color" : nexacro.ColorObject("#ee5555")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										},
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_Cal" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"color" : nexacro.ColorObject("#ee5555")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_WF_selDay" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_Cal" :
            															{
            																"self" :
            																{
            																	"mouseover" :
            																	{
            																	}
            																}
            															}
            														}
            													],
            													"self" :
            													{
            														"selected" :
            														{
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_CalW" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_Cal" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"color" : nexacro.ColorObject("#aaaaaa")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"Hcell_day" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"head" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalHead" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"color" : nexacro.ColorObject("#97999d")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"Hcell_sunday" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"head" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalHead" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"color" : nexacro.ColorObject("#ff5c6a")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"Hcell_saturday" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"head" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalHead" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"color" : nexacro.ColorObject("#4699ff")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Caladd" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"border" : nexacro.BorderObject("0px none, 1px solid #37383a, 0px none, 1px solid #37383a")
            																	},
            																	"mouseover" :
            																	{
            																		"border" : nexacro.BorderObject("0px none, 1px solid #37383a, 0px none, 1px solid #37383a")
            																	},
            																	"selected" :
            																	{
            																		"border" : nexacro.BorderObject("0px none, 1px solid #37383a, 0px none, 1px solid #37383a")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"btn_Calmore" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"padding" : nexacro.PaddingObject("3px")
            																	},
            																	"mouseover" :
            																	{
            																		"padding" : nexacro.PaddingObject("3px")
            																	},
            																	"selected" :
            																	{
            																		"padding" : nexacro.PaddingObject("3px")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_End" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"border" : nexacro.BorderObject("0px none, 1px solid #37383a, 1px solid #37383a, 1px solid #37383a")
            																	},
            																	"mouseover" :
            																	{
            																		"border" : nexacro.BorderObject("0px none, 1px solid #37383a, 1px solid #37383a, 1px solid #37383a")
            																	},
            																	"selected" :
            																	{
            																		"border" : nexacro.BorderObject("0px none, 1px solid #37383a, 1px solid #37383a, 1px solid #37383a")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Day" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#cccbd6")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#cccbd6")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#cccbd6")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Sunday" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ff808b")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ff808b")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ff808b")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Saturday" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#56a1fd")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#56a1fd")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#56a1fd")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Trailingday" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#86878c")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#86878c")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#86878c")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Trailingdaybg" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"mouseover" :
            																	{
            																	},
            																	"selected" :
            																	{
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Holiday" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("10px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ff808b"),
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 4px")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("10px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ff808b"),
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 4px")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("10px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ff808b"),
            																		"padding" : nexacro.PaddingObject("0px 0px 0px 4px")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_Today" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("12px bold \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ffffff")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("12px bold \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ffffff")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("12px bold \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ffffff")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_TodayTD" :
            						{
            							"parent" :
            							{
            								"row" :
            								{
            									"parent" :
            									{
            										"body" :
            										{
            											"parent" :
            											{
            												"Grid" :
            												{
            													"class" :
            													[
            														{
            															"grd_WF_CalMgmt" :
            															{
            																"self" :
            																{
            																	"enabled" :
            																	{
            																		"font" : nexacro.FontObject("12px bold \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ffffff")
            																	},
            																	"mouseover" :
            																	{
            																		"font" : nexacro.FontObject("12px bold \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ffffff")
            																	},
            																	"selected" :
            																	{
            																		"font" : nexacro.FontObject("12px bold \"noto\",\"Malgun Gothic\",\"Arial\""),
            																		"color" : nexacro.ColorObject("#ffffff")
            																	}
            																}
            															}
            														}
            													]
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"ico_Attachment" :
            						{
            							"parent" :
            							{
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"mouseover" :
            															{
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"focused" :
            															{
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															},
            															"readonly" :
            															{
            																"border" : nexacro.BorderObject("0px none"),
            																"padding" : nexacro.PaddingObject("0px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_CalTitle" :
            						{
            							"parent" :
            							{
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"letterSpacing" : nexacro.CSSValueObject("-0.5px")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"letterSpacing" : nexacro.CSSValueObject("-0.5px")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"letterSpacing" : nexacro.CSSValueObject("-0.5px")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																"letterSpacing" : nexacro.CSSValueObject("-0.5px")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_bg" :
            						{
            							"parent" :
            							{
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"border" : nexacro.BorderObject("1px solid #f2f2f4, 0px none, 0px none, 0px none")
            															},
            															"mouseover" :
            															{
            																"border" : nexacro.BorderObject("1px solid #f2f2f4, 0px none, 0px none, 0px none")
            															},
            															"focused" :
            															{
            																"border" : nexacro.BorderObject("1px solid #f2f2f4, 0px none, 0px none, 0px none")
            															},
            															"readonly" :
            															{
            																"border" : nexacro.BorderObject("1px solid #f2f2f4, 0px none, 0px none, 0px none")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_CalCode" :
            						{
            							"parent" :
            							{
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					},
            					{
            						"cell_CalCon" :
            						{
            							"parent" :
            							{
            								"body" :
            								{
            									"parent" :
            									{
            										"ListView" :
            										{
            											"class" :
            											[
            												{
            													"lst_WF_Calview" :
            													{
            														"self" :
            														{
            															"enabled" :
            															{
            																"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            															},
            															"mouseover" :
            															{
            																"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            															},
            															"focused" :
            															{
            																"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            															},
            															"readonly" :
            															{
            																"font" : nexacro.FontObject("11px \"noto\",\"Malgun Gothic\",\"Arial\"")
            															}
            														}
            													}
            												}
            											]
            										}
            									}
            								}
            							}
            						}
            					}
            				]
            			},
            			"cellbutton" :
            			{
            				"parent" :
            				{
            					"GridCellControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("1px solid #7b7e82"),
            								"color" : nexacro.ColorObject("#ececec"),
            								"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            							},
            							"mouseover" :
            							{
            								"border" : nexacro.BorderObject("1px solid #eaeaeb")
            							},
            							"pushed" :
            							{
            								"border" : nexacro.BorderObject("1px solid #eaeaeb")
            							},
            							"selected" :
            							{
            								"border" : nexacro.BorderObject("1px solid #eaeaeb")
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"cell" :
            					{
            						"class" :
            						[
            							{
            								"cell_WF_PointBtn01" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"self" :
            															{
            																"enabled" :
            																{
            																	"border" : nexacro.BorderObject("0px none"),
            																	"color" : nexacro.ColorObject("#ffffff")
            																}
            															}
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"cell_TF_FavMove" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"class" :
            															[
            																{
            																	"grd_TF_FavSet" :
            																	{
            																		"self" :
            																		{
            																			"enabled" :
            																			{
            																				"border" : nexacro.BorderObject("0px none")
            																			},
            																			"focused" :
            																			{
            																				"border" : nexacro.BorderObject("0px none")
            																			},
            																			"mouseover" :
            																			{
            																				"border" : nexacro.BorderObject("0px none")
            																			},
            																			"pushed" :
            																			{
            																				"border" : nexacro.BorderObject("0px none")
            																			},
            																			"selected" :
            																			{
            																				"border" : nexacro.BorderObject("0px none")
            																			}
            																		}
            																	}
            																}
            															]
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"cell_TF_Delete" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"class" :
            															[
            																{
            																	"grd_TF_FavSet" :
            																	{
            																		"self" :
            																		{
            																			"enabled" :
            																			{
            																				"border" : nexacro.BorderObject("0px none")
            																			},
            																			"focused" :
            																			{
            																				"border" : nexacro.BorderObject("0px none")
            																			},
            																			"mouseover" :
            																			{
            																				"border" : nexacro.BorderObject("0px none")
            																			},
            																			"pushed" :
            																			{
            																				"border" : nexacro.BorderObject("0px none")
            																			},
            																			"selected" :
            																			{
            																				"border" : nexacro.BorderObject("0px none")
            																			}
            																		}
            																	}
            																}
            															]
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"cell_WF_MoreBtn" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"class" :
            															[
            																{
            																	"grd_WF_Comment" :
            																	{
            																		"self" :
            																		{
            																			"enabled" :
            																			{
            																				"border" : nexacro.BorderObject("0px none"),
            																				"icon" : nexacro.UrlObject("url(\"theme://images/grd_WF_MoreBtn.png\")")
            																			},
            																			"focused" :
            																			{
            																				"border" : nexacro.BorderObject("0px none"),
            																				"icon" : nexacro.UrlObject("url(\"theme://images/grd_WF_MoreBtn.png\")")
            																			},
            																			"mouseover" :
            																			{
            																				"border" : nexacro.BorderObject("0px none"),
            																				"icon" : nexacro.UrlObject("url(\"theme://images/grd_WF_MoreBtn.png\")")
            																			},
            																			"pushed" :
            																			{
            																				"border" : nexacro.BorderObject("0px none"),
            																				"icon" : nexacro.UrlObject("url(\"theme://images/grd_WF_MoreBtn.png\")")
            																			},
            																			"selected" :
            																			{
            																				"border" : nexacro.BorderObject("0px none"),
            																				"icon" : nexacro.UrlObject("url(\"theme://images/grd_WF_MoreBtn.png\")")
            																			}
            																		}
            																	}
            																}
            															]
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"cell_Caladd" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"class" :
            															[
            																{
            																	"grd_WF_CalMgmt" :
            																	{
            																		"self" :
            																		{
            																			"enabled" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cell_Caladd.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"mouseover" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cell_Caladd.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"pushed" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cell_Caladd.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"selected" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cell_Caladd.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			}
            																		}
            																	}
            																}
            															]
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"cell_CaladdTD" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"class" :
            															[
            																{
            																	"grd_WF_CalMgmt" :
            																	{
            																		"self" :
            																		{
            																			"enabled" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cell_Caladd.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"mouseover" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cell_Caladd.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"pushed" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cell_Caladd.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"selected" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/dark/cell_Caladd.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			}
            																		}
            																	}
            																}
            															]
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"cell_Calmore" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"class" :
            															[
            																{
            																	"grd_WF_CalMgmt" :
            																	{
            																		"self" :
            																		{
            																			"enabled" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/cell_Calmore.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"mouseover" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/cell_Calmore.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"pushed" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/cell_Calmore.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"selected" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/cell_Calmore.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			}
            																		}
            																	}
            																}
            															]
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"cell_CalmoreTD" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"class" :
            															[
            																{
            																	"grd_WF_CalMgmt" :
            																	{
            																		"self" :
            																		{
            																			"enabled" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/cell_Calmore.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"mouseover" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/cell_Calmore.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"pushed" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/cell_Calmore.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			},
            																			"selected" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/cell_Calmore.png\")"),
            																				"border" : nexacro.BorderObject("0px none"),
            																				"padding" : nexacro.PaddingObject("0px")
            																			}
            																		}
            																	}
            																}
            															]
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"btn_Calmore" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"class" :
            															[
            																{
            																	"grd_WF_CalMgmt" :
            																	{
            																		"self" :
            																		{
            																			"enabled" :
            																			{
            																				"border" : nexacro.BorderObject("1px solid #e9ebf2"),
            																				"font" : nexacro.FontObject("10px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																				"color" : nexacro.ColorObject("#abadb9")
            																			},
            																			"mouseover" :
            																			{
            																				"border" : nexacro.BorderObject("1px solid #e9ebf2"),
            																				"font" : nexacro.FontObject("10px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																				"color" : nexacro.ColorObject("#abadb9")
            																			},
            																			"pushed" :
            																			{
            																				"border" : nexacro.BorderObject("1px solid #e9ebf2"),
            																				"font" : nexacro.FontObject("10px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																				"color" : nexacro.ColorObject("#abadb9")
            																			},
            																			"selected" :
            																			{
            																				"border" : nexacro.BorderObject("1px solid #e9ebf2"),
            																				"font" : nexacro.FontObject("10px \"noto\",\"Malgun Gothic\",\"Arial\""),
            																				"color" : nexacro.ColorObject("#abadb9")
            																			}
            																		}
            																	}
            																}
            															]
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"cell_Attachment" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"class" :
            															[
            																{
            																	"grd_WF_CalMgmt" :
            																	{
            																		"self" :
            																		{
            																			"enabled" :
            																			{
            																				"icon" : nexacro.UrlObject("URL(\"theme://images/cell_Attachment.png\")")
            																			}
            																		}
            																	}
            																}
            															]
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						],
            						"parent" :
            						{
            							"row" :
            							{
            								"parent" :
            								{
            									"body" :
            									{
            										"parent" :
            										{
            											"Grid" :
            											{
            												"class" :
            												[
            													{
            														"grd_LF_Menu" :
            														{
            															"self" :
            															{
            																"enabled" :
            																{
            																	"border" : nexacro.BorderObject("0px none"),
            																	"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/grd_LF_Delete.png\")")
            																},
            																"focused" :
            																{
            																	"border" : nexacro.BorderObject("0px none"),
            																	"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/grd_LF_Delete.png\")")
            																},
            																"mouseover" :
            																{
            																	"border" : nexacro.BorderObject("0px none"),
            																	"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/grd_LF_Delete.png\")")
            																},
            																"pushed" :
            																{
            																	"border" : nexacro.BorderObject("0px none"),
            																	"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/grd_LF_Delete.png\")")
            																},
            																"selected" :
            																{
            																	"border" : nexacro.BorderObject("0px none"),
            																	"padding" : nexacro.PaddingObject("0px 0px 0px 0px"),
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/grd_LF_Delete.png\")")
            																}
            															}
            														}
            													}
            												]
            											}
            										}
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"cellexpandbutton" :
            			{
            				"parent" :
            				{
            					"GridCellControl" :
            					{
            						"self" :
            						{
            							"focused" :
            							{
            							},
            							"mouseover" :
            							{
            							},
            							"pushed" :
            							{
            							},
            							"selected" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"cell" :
            					{
            						"class" :
            						[
            							{
            								"cell_WF_FilterNo" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"head" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"self" :
            															{
            																"enabled" :
            																{
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Filter.png\")")
            																},
            																"mouseover" :
            																{
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Filter.png\")")
            																},
            																"focused" :
            																{
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Filter.png\")")
            																},
            																"pushed" :
            																{
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Filter.png\")")
            																},
            																"selected" :
            																{
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_Filter.png\")")
            																}
            															}
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"cell_WF_FilterS" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"head" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"self" :
            															{
            																"enabled" :
            																{
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_FilterO.png\")")
            																},
            																"mouseover" :
            																{
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_FilterO.png\")")
            																},
            																"focused" :
            																{
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_FilterO.png\")")
            																},
            																"pushed" :
            																{
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_FilterO.png\")")
            																},
            																"selected" :
            																{
            																	"icon" : nexacro.UrlObject("URL(\"theme://images/dark/btn_WF_FilterO.png\")")
            																}
            															}
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							},
            							{
            								"cell_WF_Cal" :
            								{
            									"parent" :
            									{
            										"row" :
            										{
            											"parent" :
            											{
            												"body" :
            												{
            													"parent" :
            													{
            														"Grid" :
            														{
            															"self" :
            															{
            																"enabled" :
            																{
            																	"border" : nexacro.BorderObject("0px none")
            																},
            																"focused" :
            																{
            																	"border" : nexacro.BorderObject("0px none")
            																},
            																"mouseover" :
            																{
            																	"border" : nexacro.BorderObject("0px none")
            																},
            																"pushed" :
            																{
            																	"border" : nexacro.BorderObject("0px none")
            																},
            																"selected" :
            																{
            																	"border" : nexacro.BorderObject("0px none")
            																}
            															}
            														}
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						]
            					}
            				}
            			},
            			"treeitembutton" :
            			{
            				"parent" :
            				{
            					"celltreeitem" :
            					{
            						"parent" :
            						{
            							"GridCellControl" :
            							{
            								"self" :
            								{
            									"expand" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/grd_WF_TreeClose.png\")")
            									},
            									"collapse" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/grd_WF_TreeOpen.png\")")
            									}
            								}
            							},
            							"cell" :
            							{
            								"parent" :
            								{
            									"row" :
            									{
            										"parent" :
            										{
            											"body" :
            											{
            												"parent" :
            												{
            													"Grid" :
            													{
            														"class" :
            														[
            															{
            																"grd_LF_Menu" :
            																{
            																	"self" :
            																	{
            																		"expand" :
            																		{
            																			"icon" : nexacro.UrlObject("URL(\"theme://images/grd_LF_TreeOpen.png\")")
            																		},
            																		"collapse" :
            																		{
            																			"icon" : nexacro.UrlObject("URL(\"theme://images/grd_LF_TreeClose.png\")")
            																		}
            																	}
            																}
            															}
            														]
            													}
            												}
            											}
            										}
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"treeitemimage" :
            			{
            				"parent" :
            				{
            					"celltreeitem" :
            					{
            						"parent" :
            						{
            							"GridCellControl" :
            							{
            								"self" :
            								{
            									"expand" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/grd_WF_TreeCloseImg.png\")")
            									},
            									"collapse" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/grd_WF_TreeOpenImg.png\")")
            									},
            									"leaf" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/grd_WF_TreeLeafImg.png\")")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"celltreeline" :
            			{
            				"parent" :
            				{
            					"GridCellControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("1px dotted #999999")
            							},
            							"disabled" :
            							{
            								"border" : nexacro.BorderObject("1px dotted #aeaeae")
            							}
            						}
            					}
            				}
            			},
            			"groupboxtitle" :
            			{
            				"parent" :
            				{
            					"GroupBox" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"textPadding" : nexacro.PaddingObject("0px 5px 0px 5px")
            							},
            							"disabled" :
            							{
            								"color" : nexacro.ColorObject("#aeaeae")
            							}
            						}
            					}
            				}
            			},
            			"groupboxcontents" :
            			{
            				"parent" :
            				{
            					"GroupBox" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("1px solid #4b4d51")
            							}
            						}
            					}
            				}
            			},
            			"ImageViewer" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("0px none")
            					},
            					"disabled" :
            					{
            						"border" : nexacro.BorderObject("0px none")
            					}
            				}
            			},
            			"ListBox" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"disabled" :
            					{
            						"color" : nexacro.ColorObject("#575a5e")
            					}
            				}
            			},
            			"ListView" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("2px solid #444444, 1px solid #c9c9c9, 1px solid #c9c9c9, 1px solid #c9c9c9")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("2px solid #444444, 1px solid #c9c9c9, 1px solid #c9c9c9, 1px solid #c9c9c9")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("2px solid #444444, 1px solid #c9c9c9, 1px solid #c9c9c9, 1px solid #c9c9c9")
            					},
            					"disabled" :
            					{
            						"border" : nexacro.BorderObject("2px solid #444444, 1px solid #c9c9c9, 1px solid #c9c9c9, 1px solid #c9c9c9")
            					},
            					"readonly" :
            					{
            						"border" : nexacro.BorderObject("2px solid #444444, 1px solid #c9c9c9, 1px solid #c9c9c9, 1px solid #c9c9c9")
            					}
            				},
            				"class" :
            				[
            					{
            						"lst_WF_Calview" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					}
            				]
            			},
            			"body" :
            			{
            				"parent" :
            				{
            					"ListView" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #eaeaea, 0px none")
            							},
            							"mouseover" :
            							{
            								"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #eaeaea, 0px none")
            							},
            							"disabled" :
            							{
            								"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #eaeaea, 0px none")
            							}
            						},
            						"class" :
            						[
            							{
            								"lst_WF_Calview" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"edge" : nexacro.EdgeImageObject("url(\"theme://images/dark/cell_Calbg.png\") 10px 10px")
            										},
            										"mouseover" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"edge" : nexacro.EdgeImageObject("url(\"theme://images/dark/cell_Calbg.png\") 10px 10px")
            										},
            										"disabled" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"edge" : nexacro.EdgeImageObject("url(\"theme://images/dark/cell_Calbg.png\") 10px 10px")
            										}
            									}
            								}
            							}
            						]
            					},
            					"datepicker" :
            					{
            						"parent" :
            						{
            							"Calendar" :
            							{
            								"class" :
            								[
            									{
            										"cal_MF_Cal" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"border" : nexacro.BorderObject("0px none")
            												}
            											}
            										}
            									}
            								]
            							}
            						}
            					},
            					"Grid" :
            					{
            						"class" :
            						[
            							{
            								"grd_WF_CalMgmt" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("1px solid #37383a")
            										}
            									}
            								}
            							}
            						]
            					}
            				}
            			},
            			"MaskEdit" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51"),
            						"padding" : nexacro.PaddingObject("0px 8px 0px 0px")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"nulltext" :
            					{
            						"color" : nexacro.ColorObject("#aeaeae")
            					},
            					"disabled" :
            					{
            					}
            				},
            				"class" :
            				[
            					{
            						"output" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								}
            							}
            						}
            					},
            					{
            						"essential" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #f6e5bf"),
            									"color" : nexacro.ColorObject("#333333")
            								},
            								"mouseover" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"readonly" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"essentialSub" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #f6e5bf"),
            									"color" : nexacro.ColorObject("#333333")
            								}
            							}
            						}
            					},
            					{
            						"error" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"nulltext" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								}
            							}
            						}
            					}
            				]
            			},
            			"MaskEditControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51"),
            						"padding" : nexacro.PaddingObject("0px 8px 0px 0px")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"nulltext" :
            					{
            						"color" : nexacro.ColorObject("#aeaeae")
            					},
            					"disabled" :
            					{
            					}
            				}
            			},
            			"Menu" :
            			{
            				"self" :
            				{
            					"mouseover" :
            					{
            					},
            					"focused" :
            					{
            					}
            				}
            			},
            			"prevbutton" :
            			{
            				"parent" :
            				{
            					"Menu" :
            					{
            						"self" :
            						{
            							"focused" :
            							{
            							},
            							"mouseover" :
            							{
            							},
            							"pushed" :
            							{
            							},
            							"selected" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"PopupMenu" :
            					{
            						"self" :
            						{
            							"focused" :
            							{
            							},
            							"mouseover" :
            							{
            							},
            							"pushed" :
            							{
            							},
            							"selected" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"Tab" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_PreBtn.png\")"),
            								"padding" : nexacro.PaddingObject("3px")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_PreBtn.png\")"),
            								"padding" : nexacro.PaddingObject("3px")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_PreBtnO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_PreBtnO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_PreBtnO.png\")")
            							}
            						}
            					},
            					"head" :
            					{
            						"parent" :
            						{
            							"datepicker" :
            							{
            								"parent" :
            								{
            									"Calendar" :
            									{
            										"class" :
            										[
            											{
            												"cal_MF_Cal" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalPrev.png\")")
            														},
            														"focused" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalPrev.png\")")
            														},
            														"mouseover" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalPrevO.png\")")
            														},
            														"pushed" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalPrevO.png\")")
            														},
            														"selected" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalPrevO.png\")")
            														},
            														"disabled" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalPrev.png\")")
            														}
            													}
            												}
            											}
            										]
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"nextbutton" :
            			{
            				"parent" :
            				{
            					"Menu" :
            					{
            						"self" :
            						{
            							"focused" :
            							{
            							},
            							"mouseover" :
            							{
            							},
            							"pushed" :
            							{
            							},
            							"selected" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"PopupMenu" :
            					{
            						"self" :
            						{
            							"focused" :
            							{
            							},
            							"mouseover" :
            							{
            							},
            							"pushed" :
            							{
            							},
            							"selected" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"Tab" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_NxtBtn.png\")"),
            								"padding" : nexacro.PaddingObject("3px")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_NxtBtn.png\")"),
            								"padding" : nexacro.PaddingObject("3px")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_NxtBtnO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_NxtBtnO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_NxtBtnO.png\")")
            							}
            						}
            					},
            					"head" :
            					{
            						"parent" :
            						{
            							"datepicker" :
            							{
            								"parent" :
            								{
            									"Calendar" :
            									{
            										"class" :
            										[
            											{
            												"cal_MF_Cal" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalNext.png\")")
            														},
            														"focused" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalNext.png\")")
            														},
            														"mouseover" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalNextO.png\")")
            														},
            														"pushed" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalNextO.png\")")
            														},
            														"selected" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalNextO.png\")")
            														},
            														"disabled" :
            														{
            															"icon" : nexacro.UrlObject("URL(\"theme://images/btn_MF_CalNext.png\")")
            														}
            													}
            												}
            											}
            										]
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"menuitem" :
            			{
            				"parent" :
            				{
            					"Menu" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"padding" : nexacro.PaddingObject("0px 25px 0px 25px"),
            								"color" : nexacro.ColorObject("#ffffff"),
            								"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            							},
            							"mouseover" :
            							{
            								"color" : nexacro.ColorObject("#31bff2")
            							},
            							"selected" :
            							{
            								"color" : nexacro.ColorObject("#31bff2")
            							},
            							"focused" :
            							{
            								"color" : nexacro.ColorObject("#31bff2")
            							},
            							"disabled" :
            							{
            								"color" : nexacro.ColorObject("#5b6289")
            							}
            						}
            					}
            				}
            			},
            			"PopupMenu" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #646982")
            					}
            				}
            			},
            			"popupmenuitem" :
            			{
            				"parent" :
            				{
            					"PopupMenu" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"padding" : nexacro.PaddingObject("5px 10px 5px 10px")
            							},
            							"focused" :
            							{
            								"padding" : nexacro.PaddingObject("5px 10px 5px 10px")
            							},
            							"disabled" :
            							{
            								"padding" : nexacro.PaddingObject("5px 10px 5px 10px")
            							},
            							"mouseover" :
            							{
            							},
            							"selected" :
            							{
            							}
            						}
            					},
            					"PopupMenuControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"padding" : nexacro.PaddingObject("0px 10px 0px 0px"),
            								"color" : nexacro.ColorObject("#333333")
            							},
            							"focused" :
            							{
            								"padding" : nexacro.PaddingObject("0px 10px 0px 0px"),
            								"color" : nexacro.ColorObject("#333333")
            							},
            							"disabled" :
            							{
            								"padding" : nexacro.PaddingObject("0px 10px 0px 0px"),
            								"color" : nexacro.ColorObject("#333333")
            							},
            							"mouseover" :
            							{
            								"color" : nexacro.ColorObject("#333333")
            							},
            							"selected" :
            							{
            								"color" : nexacro.ColorObject("#333333")
            							}
            						}
            					}
            				}
            			},
            			"popupmenuitemcheckbox" :
            			{
            				"parent" :
            				{
            					"popupmenuitem" :
            					{
            						"parent" :
            						{
            							"PopupMenu" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme:://images/mnu_WF_PmnuChk.png\")")
            									},
            									"mouseover" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme:://images/mnu_WF_PmnuChk.png\")")
            									},
            									"disabled" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme:://images/mnu_WF_PmnuChk.png\")")
            									}
            								}
            							},
            							"PopupMenuControl" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/mnu_WF_PmnuChk.png\")"),
            										"padding" : nexacro.PaddingObject("5px 0px 5px 5px")
            									},
            									"mouseover" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/mnu_WF_PmnuChk.png\")"),
            										"padding" : nexacro.PaddingObject("5px 0px 5px 5px")
            									},
            									"disabled" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/mnu_WF_PmnuChk.png\")"),
            										"padding" : nexacro.PaddingObject("5px 0px 5px 5px")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"popupmenuitemhotkeytext" :
            			{
            				"parent" :
            				{
            					"popupmenuitem" :
            					{
            						"parent" :
            						{
            							"PopupMenu" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"color" : nexacro.ColorObject("#dddddd")
            									},
            									"mouseover" :
            									{
            										"color" : nexacro.ColorObject("#dddddd")
            									},
            									"disabled" :
            									{
            										"color" : nexacro.ColorObject("#dddddd")
            									}
            								}
            							},
            							"PopupMenuControl" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"padding" : nexacro.PaddingObject("0px 10px 0px 0px"),
            										"color" : nexacro.ColorObject("#aaaaaa")
            									},
            									"mouseover" :
            									{
            										"padding" : nexacro.PaddingObject("0px 10px 0px 0px"),
            										"color" : nexacro.ColorObject("#aaaaaa")
            									},
            									"disabled" :
            									{
            										"padding" : nexacro.PaddingObject("0px 10px 0px 0px"),
            										"color" : nexacro.ColorObject("#aaaaaa")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"popupmenuitemtext" :
            			{
            				"parent" :
            				{
            					"popupmenuitem" :
            					{
            						"parent" :
            						{
            							"PopupMenu" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"padding" : nexacro.PaddingObject("0px 20px 0px 0px"),
            										"color" : nexacro.ColorObject("#333333")
            									},
            									"mouseover" :
            									{
            										"padding" : nexacro.PaddingObject("0px 20px 0px 0px"),
            										"color" : nexacro.ColorObject("#333333")
            									},
            									"disabled" :
            									{
            										"padding" : nexacro.PaddingObject("0px 20px 0px 0px"),
            										"color" : nexacro.ColorObject("#333333")
            									}
            								}
            							},
            							"PopupMenuControl" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"padding" : nexacro.PaddingObject("5px 0px 5px 0px")
            									},
            									"mouseover" :
            									{
            										"padding" : nexacro.PaddingObject("5px 0px 5px 0px")
            									},
            									"disabled" :
            									{
            										"padding" : nexacro.PaddingObject("5px 0px 5px 0px")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"popupmenuitemexpandimage" :
            			{
            				"parent" :
            				{
            					"popupmenuitem" :
            					{
            						"parent" :
            						{
            							"PopupMenu" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/pmnu_WF_SelBtn.png\")"),
            										"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            									},
            									"mouseover" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/pmnu_WF_SelBtn.png\")"),
            										"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            									},
            									"disabled" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/pmnu_WF_SelBtn.png\")"),
            										"padding" : nexacro.PaddingObject("0px 0px 0px 0px")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"PopupMenuControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #646982")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #646982")
            					},
            					"disabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #646982")
            					}
            				}
            			},
            			"ProgressBar" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"disabled" :
            					{
            					}
            				}
            			},
            			"ProgressBarControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"disabled" :
            					{
            					}
            				}
            			},
            			"progressbaritem" :
            			{
            				"parent" :
            				{
            					"ProgressBar" :
            					{
            						"self" :
            						{
            							"mouseover" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"ProgressBarControl" :
            					{
            						"self" :
            						{
            							"mouseover" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					}
            				}
            			},
            			"progressbartext" :
            			{
            				"parent" :
            				{
            					"ProgressBar" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"padding" : nexacro.PaddingObject("0px 10px 0px 10px"),
            								"color" : nexacro.ColorObject("#ffffff")
            							},
            							"mouseover" :
            							{
            								"padding" : nexacro.PaddingObject("0px 10px 0px 10px"),
            								"color" : nexacro.ColorObject("#ffffff")
            							},
            							"disabled" :
            							{
            								"padding" : nexacro.PaddingObject("0px 10px 0px 10px"),
            								"color" : nexacro.ColorObject("#ffffff")
            							}
            						}
            					},
            					"ProgressBarControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"padding" : nexacro.PaddingObject("0px 10px 0px 10px"),
            								"color" : nexacro.ColorObject("#ffffff")
            							},
            							"mouseover" :
            							{
            								"padding" : nexacro.PaddingObject("0px 10px 0px 10px"),
            								"color" : nexacro.ColorObject("#ffffff")
            							},
            							"disabled" :
            							{
            								"padding" : nexacro.PaddingObject("0px 10px 0px 10px"),
            								"color" : nexacro.ColorObject("#ffffff")
            							}
            						}
            					}
            				}
            			},
            			"progressstartcap" :
            			{
            				"parent" :
            				{
            					"ProgressBar" :
            					{
            						"self" :
            						{
            							"mouseover" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"ProgressBarControl" :
            					{
            						"self" :
            						{
            							"mouseover" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					}
            				}
            			},
            			"progressendcap" :
            			{
            				"parent" :
            				{
            					"ProgressBar" :
            					{
            						"self" :
            						{
            							"mouseover" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					},
            					"ProgressBarControl" :
            					{
            						"self" :
            						{
            							"mouseover" :
            							{
            							},
            							"disabled" :
            							{
            							}
            						}
            					}
            				}
            			},
            			"radioitem" :
            			{
            				"parent" :
            				{
            					"Radio" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_Item.png\")"),
            								"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            							},
            							"readonly" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_Item.png\")"),
            								"textPadding" : nexacro.PaddingObject("0px 16px 0px 6px")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_ItemO.png\")")
            							},
            							"mouseover_selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_ItemSO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_ItemS.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_ItemD.png\")")
            							},
            							"disabled_selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_ItemSD.png\")")
            							}
            						},
            						"class" :
            						[
            							{
            								"essential" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_ItemES.png\")")
            										},
            										"mouseover" :
            										{
            											"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_ItemSES.png\")")
            										},
            										"mouseover_selected" :
            										{
            											"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_ItemSES.png\")")
            										},
            										"selected" :
            										{
            											"icon" : nexacro.UrlObject("URL(\"theme://images/rdo_WF_ItemSES.png\")")
            										}
            									}
            								}
            							}
            						]
            					}
            				}
            			},
            			"Splitter" :
            			{
            				"class" :
            				[
            					{
            						"spt_WF_V" :
            						{
            							"self" :
            							{
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"spt_WF_H" :
            						{
            							"self" :
            							{
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					}
            				]
            			},
            			"Spin" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"disabled" :
            					{
            					}
            				},
            				"class" :
            				[
            					{
            						"essential" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #f6e5bf"),
            									"color" : nexacro.ColorObject("#333333")
            								},
            								"mouseover" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"readonly" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"error" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								}
            							}
            						}
            					}
            				]
            			},
            			"spindownbutton" :
            			{
            				"parent" :
            				{
            					"Spin" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/spn_WF_DownBtn.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/spn_WF_DownBtnD.png\")")
            							}
            						},
            						"class" :
            						[
            							{
            								"essential" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            										}
            									}
            								}
            							}
            						]
            					},
            					"SpinControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtn.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtn.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_DownBtnD.png\")")
            							}
            						}
            					}
            				}
            			},
            			"spinupbutton" :
            			{
            				"parent" :
            				{
            					"Spin" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/spn_WF_UpBtn.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/dark/spn_WF_UpBtnD.png\")")
            							}
            						},
            						"class" :
            						[
            							{
            								"essential" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            										}
            									}
            								}
            							}
            						]
            					},
            					"SpinControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtn.png\")")
            							},
            							"focused" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtn.png\")")
            							},
            							"mouseover" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            							},
            							"pushed" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            							},
            							"selected" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnO.png\")")
            							},
            							"disabled" :
            							{
            								"icon" : nexacro.UrlObject("URL(\"theme://images/spn_WF_UpBtnD.png\")")
            							}
            						}
            					}
            				}
            			},
            			"spinedit" :
            			{
            				"parent" :
            				{
            					"Spin" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 0px")
            							},
            							"mouseover" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 0px")
            							},
            							"focused" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 0px")
            							},
            							"readonly" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 0px")
            							},
            							"nulltext" :
            							{
            								"color" : nexacro.ColorObject("#aaaaaa")
            							},
            							"disabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"color" : nexacro.ColorObject("#aeaeae")
            							}
            						}
            					},
            					"SpinControl" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 8px 0px 0px"),
            								"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            							},
            							"mouseover" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 4px 0px"),
            								"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            							},
            							"focused" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 4px 0px"),
            								"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            							},
            							"readonly" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"padding" : nexacro.PaddingObject("0px 0px 4px 0px"),
            								"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            							},
            							"nulltext" :
            							{
            								"color" : nexacro.ColorObject("#aaaaaa")
            							},
            							"disabled" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"color" : nexacro.ColorObject("#aeaeae")
            							}
            						}
            					}
            				}
            			},
            			"SpinControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("0px none")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("0px none")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("0px none")
            					},
            					"disabled" :
            					{
            					}
            				}
            			},
            			"Static" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"wordWrap" : "char"
            					},
            					"disabled" :
            					{
            						"color" : nexacro.ColorObject("#575a5e")
            					}
            				},
            				"class" :
            				[
            					{
            						"sta_TF_Bg" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #393a3c, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"sta_TF_Logo" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 16px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_TF_User" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#AEB2C5"),
            									"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_LF_Title" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"font" : nexacro.FontObject("bold 16px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_BT_Default" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#222222"),
            									"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_BT_Num" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 3px 0px 0px"),
            									"color" : nexacro.ColorObject("#222222"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_BF_Error" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            									"color" : nexacro.ColorObject("#e10000"),
            									"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_BF_Info" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            									"color" : nexacro.ColorObject("#2756b6"),
            									"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_BF_Warning" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 16px"),
            									"color" : nexacro.ColorObject("#f28005"),
            									"font" : nexacro.FontObject("12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_MF_tile" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 16px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_MF_tile2" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 16px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_MF_Logo" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#379DDD"),
            									"font" : nexacro.FontObject("bold 16px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_MF_Title" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#94969B"),
            									"font" : nexacro.FontObject("15px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"letterSpacing" : nexacro.CSSValueObject("-1px")
            								}
            							}
            						}
            					},
            					{
            						"sta_MF_Menu" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("2px solid #C8C9CC, 0px none, 1px solid #3A3B3D, 0px none"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 25px"),
            									"font" : nexacro.FontObject("bold 23px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"letterSpacing" : nexacro.CSSValueObject("-1px")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_Location" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#8e919c"),
            									"font" : nexacro.FontObject("16px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"wordWrap" : "none"
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_SchLabel" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 10px 0px 28px"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"letterSpacing" : nexacro.CSSValueObject("-1px")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_SchLabelSub" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 10px 0px 28px"),
            									"color" : nexacro.ColorObject("#7d859b"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"letterSpacing" : nexacro.CSSValueObject("-1px")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_PostTitle" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 0px none, 1px solid #ececec, 0px none"),
            									"font" : nexacro.FontObject("bold 20px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"letterSpacing" : nexacro.CSSValueObject("-1px")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_PostAttach" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #393A3C, 0px none, 0px none, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_PostLabel" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"letterSpacing" : nexacro.CSSValueObject("-1px"),
            									"color" : nexacro.ColorObject("#888888")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_PostDate" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"font" : nexacro.FontObject("14px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"letterSpacing" : nexacro.CSSValueObject("-1px"),
            									"color" : nexacro.ColorObject("#888888")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_PostID" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 22px"),
            									"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"color" : nexacro.ColorObject("#ececec")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_Bold" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_Label" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #47484b"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 12px"),
            									"color" : nexacro.ColorObject("#afafaf"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_LabelBg" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #393a3c"),
            									"padding" : nexacro.PaddingObject("0px 0px 0px 12px"),
            									"color" : nexacro.ColorObject("#ffffff")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_Title" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 10px 16px"),
            									"font" : nexacro.FontObject("bold 16px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_SubTitle" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 10px 16px"),
            									"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_Titlesm" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 10px 16px"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_Comment" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 22px"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"color" : nexacro.ColorObject("#000000")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_Dot" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 8px"),
            									"font" : nexacro.FontObject("bold 12px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"color" : nexacro.ColorObject("#F3F3F3")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_ExTitle" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 13px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_Cal" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"font" : nexacro.FontObject("bold 14px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_LOGIN_Logo" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"edge" : nexacro.EdgeImageObject("url('theme://images/sta_LOGIN_Logo.png') 0px")
            								}
            							}
            						}
            					},
            					{
            						"sta_LOGIN_Id" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 0px none, 2px solid #e9ebf3, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"sta_LOGIN_Pw" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 0px none, 2px solid #e9ebf3, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"sta_LOGIN_Language" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none, 0px none, 2px solid #e9ebf3, 0px none")
            								}
            							}
            						}
            					},
            					{
            						"sta_POP_Title" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 15px \"noto\",\"Malgun Gothic\",\"Arial\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GSize" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#000000"),
            									"font" : nexacro.FontObject("bold 12px \"Dotum\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GTxt01" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"font" : nexacro.FontObject("bold 20px \"Tahoma\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GTxt02" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"font" : nexacro.FontObject("bold 12px \"Dotum\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GTxt02B" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#333333"),
            									"font" : nexacro.FontObject("bold 12px \"Tahoma\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GTxt03" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#8e96b5"),
            									"font" : nexacro.FontObject("11px \"Tahoma\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GTxt04" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#ff000c"),
            									"font" : nexacro.FontObject("11px \"Tahoma\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GTxt05" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#ff7200"),
            									"font" : nexacro.FontObject("bold 12px/20px \"Dotum\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GTxt06" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#ee5555"),
            									"font" : nexacro.FontObject("12px/20px \"Dotum\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GBar" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"padding" : nexacro.PaddingObject("0px 0px 0px 55px"),
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 35px \"Tahoma\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GNum" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"color" : nexacro.ColorObject("#ffffff"),
            									"font" : nexacro.FontObject("bold 30px \"Tahoma\"")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_Box" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3a3b3d")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GLineV" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"edge" : nexacro.EdgeImageObject("URL('theme://images/sta_WF_GLineV.png') 5px 2px")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GLineH" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"edge" : nexacro.EdgeImageObject("URL('theme://images/sta_WF_GLineH.png') 5px 2px")
            								}
            							}
            						}
            					},
            					{
            						"sta_WF_GBg02" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #333333"),
            									"padding" : nexacro.PaddingObject("8px"),
            									"color" : nexacro.ColorObject("#333333")
            								}
            							}
            						}
            					},
            					{
            						"sta_CalMonth" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"font" : nexacro.FontObject("26px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"color" : nexacro.ColorObject("#bbbfc7"),
            									"letterSpacing" : nexacro.CSSValueObject("-1px")
            								}
            							}
            						}
            					},
            					{
            						"sta_CalTitle" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"font" : nexacro.FontObject("16px \"noto\",\"Malgun Gothic\",\"Arial\""),
            									"color" : nexacro.ColorObject("#8e919c")
            								}
            							}
            						}
            					}
            				]
            			},
            			"StepControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"padding" : nexacro.PaddingObject("0px 0px 20px 0px")
            					}
            				}
            			},
            			"stepitem" :
            			{
            				"parent" :
            				{
            					"StepControl" :
            					{
            						"self" :
            						{
            							"selected" :
            							{
            							}
            						}
            					}
            				}
            			},
            			"tabbuttonitem" :
            			{
            				"parent" :
            				{
            					"Tab" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("0px none, 1px solid #4b4d51,  0px none, 0px none"),
            								"color" : nexacro.ColorObject("#ffffff")
            							},
            							"focused" :
            							{
            								"border" : nexacro.BorderObject("0px none, 1px solid #4b4d51,  0px none, 0px none"),
            								"color" : nexacro.ColorObject("#ffffff")
            							},
            							"mouseover" :
            							{
            								"border" : nexacro.BorderObject("0px none, 1px solid #4b4d51,  0px none, 0px none"),
            								"color" : nexacro.ColorObject("#ffffff")
            							},
            							"selected" :
            							{
            								"border" : nexacro.BorderObject("0px none"),
            								"color" : nexacro.ColorObject("#ffffff")
            							},
            							"disabled" :
            							{
            								"color" : nexacro.ColorObject("#575a5e")
            							}
            						}
            					}
            				}
            			},
            			"tabbuttonitemtext" :
            			{
            				"parent" :
            				{
            					"tabbuttonitem" :
            					{
            						"parent" :
            						{
            							"Tab" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"padding" : nexacro.PaddingObject("0px 20px 0px 20px"),
            										"font" : nexacro.FontObject("bold 14px/32px \"noto\",\"Malgun Gothic\",\"Arial\"")
            									},
            									"mouseover" :
            									{
            									},
            									"disabled" :
            									{
            										"color" : nexacro.ColorObject("#575a5e")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"extrabutton" :
            			{
            				"parent" :
            				{
            					"tabbuttonitem" :
            					{
            						"parent" :
            						{
            							"Tab" :
            							{
            								"self" :
            								{
            									"enabled" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_X.png\")"),
            										"padding" : nexacro.PaddingObject("0px 10px 0px 0px")
            									},
            									"mouseover" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_X.png\")"),
            										"padding" : nexacro.PaddingObject("0px 10px 0px 0px")
            									},
            									"focused" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_X.png\")"),
            										"padding" : nexacro.PaddingObject("0px 10px 0px 0px")
            									},
            									"pushed" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_X.png\")"),
            										"padding" : nexacro.PaddingObject("0px 10px 0px 0px")
            									},
            									"selected" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_X.png\")"),
            										"padding" : nexacro.PaddingObject("0px 10px 0px 0px")
            									},
            									"disabled" :
            									{
            										"icon" : nexacro.UrlObject("URL(\"theme://images/tab_WF_X.png\")"),
            										"padding" : nexacro.PaddingObject("0px 10px 0px 0px")
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"tabpage" :
            			{
            				"parent" :
            				{
            					"Tab" :
            					{
            						"self" :
            						{
            							"enabled" :
            							{
            								"border" : nexacro.BorderObject("1px solid #393a3c, 0px none, 0px none, 0px none")
            							},
            							"mouseover" :
            							{
            								"border" : nexacro.BorderObject("1px solid #393a3c, 0px none, 0px none, 0px none")
            							},
            							"focused" :
            							{
            								"border" : nexacro.BorderObject("1px solid #393a3c, 0px none, 0px none, 0px none")
            							},
            							"disabled" :
            							{
            								"border" : nexacro.BorderObject("1px solid #393a3c, 0px none, 0px none, 0px none")
            							}
            						}
            					}
            				}
            			},
            			"TextArea" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51"),
            						"padding" : nexacro.PaddingObject("8px 8px 0px 8px"),
            						"wordWrap" : "char",
            						"font" : nexacro.FontObject("13px/18px \"noto\",\"Malgun Gothic\",\"Arial\"")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"nulltext" :
            					{
            						"color" : nexacro.ColorObject("#aaaaaa")
            					},
            					"disabled" :
            					{
            					}
            				},
            				"class" :
            				[
            					{
            						"output" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("0px none"),
            									"padding" : nexacro.PaddingObject("0px"),
            									"color" : nexacro.ColorObject("#ebebeb")
            								}
            							}
            						}
            					},
            					{
            						"essential" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #f6e5bf"),
            									"color" : nexacro.ColorObject("#333333")
            								},
            								"mouseover" :
            								{
            								},
            								"focused" :
            								{
            								},
            								"readonly" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"error" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"nulltext" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								},
            								"disabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #ee5555")
            								}
            							}
            						}
            					},
            					{
            						"txt_POP_Text" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #dddddd")
            								},
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #dddddd")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #dddddd")
            								},
            								"readonly" :
            								{
            									"border" : nexacro.BorderObject("1px solid #dddddd")
            								}
            							}
            						}
            					}
            				]
            			},
            			"TextAreaControl" :
            			{
            				"self" :
            				{
            					"enabled" :
            					{
            						"border" : nexacro.BorderObject("1px solid #4b4d51"),
            						"padding" : nexacro.PaddingObject("8px 8px 0px 8px"),
            						"wordWrap" : "char",
            						"font" : nexacro.FontObject("13px/18px \"noto\",\"Malgun Gothic\",\"Arial\"")
            					},
            					"mouseover" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"focused" :
            					{
            						"border" : nexacro.BorderObject("1px solid #ffffff"),
            						"color" : nexacro.ColorObject("#333333")
            					},
            					"readonly" :
            					{
            					},
            					"nulltext" :
            					{
            						"color" : nexacro.ColorObject("#aaaaaa")
            					},
            					"disabled" :
            					{
            					}
            				}
            			},
            			"PopupDiv" :
            			{
            				"class" :
            				[
            					{
            						"pdiv_MDI_Bg" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646982")
            								}
            							}
            						}
            					},
            					{
            						"pdiv_LF_Sch" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #444444")
            								}
            							}
            						}
            					},
            					{
            						"pdiv_WF_List" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #a0a4b0")
            								}
            							}
            						}
            					},
            					{
            						"pdiv_WF_Bg" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #646982")
            								}
            							}
            						}
            					}
            				]
            			},
            			"vscrollbar" :
            			{
            				"parent" :
            				{
            					"Grid" :
            					{
            						"class" :
            						[
            							{
            								"grd_LF_Menu" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"mouseover" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"disabled" :
            										{
            										}
            									}
            								}
            							},
            							{
            								"grd_MDI_List" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"mouseover" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"disabled" :
            										{
            										}
            									}
            								}
            							},
            							{
            								"grd_TF_FavSet" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"mouseover" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"disabled" :
            										{
            										}
            									}
            								}
            							},
            							{
            								"grd_MF_Notice" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"mouseover" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"disabled" :
            										{
            										}
            									}
            								}
            							},
            							{
            								"grd_WF_CalMgmt" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("1px solid #37383a, 0px none, 1px solid #37383a, 0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"mouseover" :
            										{
            											"border" : nexacro.BorderObject("1px solid #37383a, 0px none, 1px solid #37383a, 0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"disabled" :
            										{
            										}
            									}
            								}
            							}
            						]
            					},
            					"Form" :
            					{
            						"class" :
            						[
            							{
            								"frm_MF_MegaMenu" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"mouseover" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"disabled" :
            										{
            										}
            									}
            								}
            							}
            						]
            					},
            					"ListView" :
            					{
            						"class" :
            						[
            							{
            								"lst_WF_Calview" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										},
            										"mouseover" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"padding" : nexacro.PaddingObject("0px 5px")
            										}
            									}
            								}
            							}
            						]
            					}
            				}
            			},
            			"datepicker" :
            			{
            				"parent" :
            				{
            					"Calendar" :
            					{
            						"class" :
            						[
            							{
            								"cal_MF_Cal" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("0px none")
            										}
            									}
            								}
            							}
            						]
            					}
            				}
            			},
            			"head" :
            			{
            				"parent" :
            				{
            					"datepicker" :
            					{
            						"parent" :
            						{
            							"Calendar" :
            							{
            								"class" :
            								[
            									{
            										"cal_MF_Cal" :
            										{
            											"self" :
            											{
            												"enabled" :
            												{
            													"border" : nexacro.BorderObject("0px none")
            												}
            											}
            										}
            									}
            								]
            							}
            						}
            					},
            					"Grid" :
            					{
            						"class" :
            						[
            							{
            								"grd_WF_Cal" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("0px none"),
            											"color" : nexacro.ColorObject("#333333")
            										}
            									}
            								}
            							},
            							{
            								"grd_WF_CalHead" :
            								{
            									"self" :
            									{
            										"enabled" :
            										{
            											"border" : nexacro.BorderObject("0px none")
            										}
            									}
            								}
            							}
            						]
            					}
            				}
            			},
            			"monthstatic" :
            			{
            				"parent" :
            				{
            					"head" :
            					{
            						"parent" :
            						{
            							"datepicker" :
            							{
            								"parent" :
            								{
            									"Calendar" :
            									{
            										"class" :
            										[
            											{
            												"cal_MF_Cal" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"color" : nexacro.ColorObject("#ececec"),
            															"font" : nexacro.FontObject("bold 16px \"Myriad Pro\"")
            														}
            													}
            												}
            											}
            										]
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"yearstatic" :
            			{
            				"parent" :
            				{
            					"head" :
            					{
            						"parent" :
            						{
            							"datepicker" :
            							{
            								"parent" :
            								{
            									"Calendar" :
            									{
            										"class" :
            										[
            											{
            												"cal_MF_Cal" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"color" : nexacro.ColorObject("#ececec"),
            															"font" : nexacro.FontObject("bold 16px \"Myriad Pro\"")
            														}
            													}
            												}
            											}
            										]
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"dayitem" :
            			{
            				"parent" :
            				{
            					"body" :
            					{
            						"parent" :
            						{
            							"datepicker" :
            							{
            								"parent" :
            								{
            									"Calendar" :
            									{
            										"class" :
            										[
            											{
            												"cal_MF_Cal" :
            												{
            													"self" :
            													{
            														"enabled" :
            														{
            															"font" : nexacro.FontObject("bold 13px \"Myriad Pro\"")
            														},
            														"day" :
            														{
            															"color" : nexacro.ColorObject("#ececec")
            														},
            														"mouseover" :
            														{
            															"color" : nexacro.ColorObject("#3683e2")
            														},
            														"selected" :
            														{
            															"color" : nexacro.ColorObject("#A5E1F3")
            														},
            														"today" :
            														{
            															"color" : nexacro.ColorObject("#3e8ded")
            														}
            													}
            												}
            											}
            										]
            									}
            								}
            							}
            						}
            					}
            				}
            			},
            			"Div" :
            			{
            				"class" :
            				[
            					{
            						"div_MF_H" :
            						{
            							"self" :
            							{
            								"focused" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								},
            								"mouseover" :
            								{
            									"border" : nexacro.BorderObject("1px solid #eaeaeb")
            								},
            								"disabled" :
            								{
            								}
            							}
            						}
            					},
            					{
            						"div_WF_Search" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3a3b3d, 1px solid #3a3b3d, 1px solid #101112, 1px solid #3a3b3d")
            								}
            							}
            						}
            					},
            					{
            						"div_WF_SchSub" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #3a3b3d")
            								}
            							}
            						}
            					},
            					{
            						"div_CalHead" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("0px none")
            								}
            							}
            						}
            					},
            					{
            						"div_CalView" :
            						{
            							"self" :
            							{
            								"enabled" :
            								{
            									"border" : nexacro.BorderObject("1px solid #393a3c")
            								}
            							}
            						}
            					}
            				]
            			}
		},
		{
            "includeStatusMap" : true,
            			"applyMarkupValidation" : true
		}
		);
		var imgcache = nexacro._getImageCacheMaps();

	};
}
)();
